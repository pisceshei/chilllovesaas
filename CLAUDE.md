...
15. **提交前復核鐵律（2026-08-17 使用者裁定；違反＝該輪作廢）**：
    **所有階段、所有 PR、所有驗收回應輪，push 到 GitHub 前必須做「逐項復核」**：
    - **15.1 逐項對照**：把本輪要處理的**全部**意見（含所有驗收方、含最新時刻的每一則
      判詞）逐條列成清單，逐項記錄處置：**宣稱已修復者**核對「diff 裡有對應 hunk」；
      走清法②裁定不修者核對「DECISIONS／PR 描述條目存在」、③證偽者核對「**證據或其可存取引用存在**」（僅有「已證偽」登記＝**缺項** （2026-08-17 更正，PR #53 第 5 輪））、⚪ 者核對「登記存在」（此三類不產生 hunk，
      （2026-08-17 更正，PR #53 首輪）：原文「全部意見各有 hunk」會把合法清法變成違規）。
      「diff」＝**已 commit 的差異**（HEAD 對 PR base，核對時點見 15.4）不是工作樹（2026-08-17 更正，PR #53 第 3/6 輪：句位自「此三類」中段移出）。**缺一項不得 push**＝清單存在「既無對應 hunk、也無合規②③⚪登記」的項目。
    - **15.2 時序復查**：push 前**重新拉取 PR 頁最新留言**——判詞可能在你動工期間新貼
      （2026-08-17 實測：19:27 判詞 vs 19:29 push 交錯，整批 🔴 漏收一輪）。
    - **15.3 宣稱與實物同刻**：「N 項全收」定論**只寫在核對完成後的 PR 留言**；
      commit message 與**隨 commit 入庫的 worklog** 皆不作全收定論（worklog 記處置清單
      即可）——兩者都在核對完成前定稿（（2026-08-17 更正，PR #53 第 5 輪）：第 4 輪版本一邊禁 commit
      message、一邊允許 worklog，自相矛盾）。宣稱前 N 必須等於 15.1 清單長度且
      逐項打勾（同日兩次「稱全收而漏項」即本條立法事故）。
    - **15.4 順序**：清單建立＋修復完成 → 全部閘門逐支親眼看退出碼 → **commit** →
      **15.1 逐項 hunk 核對（對 `git diff origin/main...HEAD`（PR base 2026-08-17 更正，PR #53 第 7 輪：原寫 `base` 非可解析 ref）——commit 後才有已提交差異可核**
      （2026-08-17 更正，PR #53 第 3 輪）：原順序把核對排在 commit 前、照字面永遠不成立；核對缺項 ⇒ 補修復
      重跑閘門再 commit）→
      **15.2 重拉留言（push 前最後一個動作**；「留言」含**兩種形式**：bot 判詞留言＋Codex inline review；**首次 push（PR 尚未存在）豁免 15.2**，開 PR 後首輪意見照常進 15.1（2026-08-17 更正，PR #53 第 2 輪）——重拉不動檔，與閘門相容；閘門與 commit
      耗時分鐘級、與立法事故①的 2 分鐘窗同量級，排在閘門前防不住它 （2026-08-17 更正，PR #53 首輪））→
      有新判詞 ⇒ **回 15.1 重來**；無 ⇒ push。閘門後再動任何檔案＝回到 15.1 重來。
...
- 🔴 **🔴＋🟡 全清驗收（2026-08-16 使用者裁定，取代先前「🟡 不擋通過」）**：
  ...
  `docs/specs/91-pit-register.md` 坑登記簿（該檔尚未建立，由排查階段 PR-E1 引入；
  建立前登記於 PR 描述）。
