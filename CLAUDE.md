...
15. **提交前復核鐵律（2026-08-17 使用者裁定；違反＝該輪作廢）**：
    **所有階段、所有 PR、所有驗收回應輪，push 到 GitHub 前必須做「逐項復核」**：
    - **15.1 逐項對照**：把本輪要處理的**全部**意見（含所有驗收方、含最新時刻的每一則
      ...
      走清法②裁定不修者核對「DECISIONS／PR 描述條目存在」、③證偽者核對「**證據已附**於留言或 PR 描述」、⚪ 者核對「登記存在」；「diff」＝**已 commit 的差異**（HEAD 對 PR base，核對時點見 15.4 （2026-08-17 更正，PR #53 第 3 輪）），不是工作樹（此三類不產生 hunk，
    ...
    - **15.3 宣稱與實物同刻**（宣稱寫在 **commit 後核對完成的 worklog／PR 留言**；
      commit message 不作「N 項全收」宣稱——核對依據是已提交差異、建 commit 前不可能
      完成核對 （2026-08-17 更正，PR #53 第 4 輪））：worklog/commit message 寫「N 項全收」前，N 必須等於
      15.1 清單長度且逐項打勾（同日兩次「稱全收而漏項」即本條立法事故）。
