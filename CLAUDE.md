...
15. **提交前復核鐵律（2026-08-17 使用者裁定；違反＝該輪作廢）**：
    **所有階段、所有 PR、所有驗收回應輪，push 到 GitHub 前必須做「逐項復核」**：
    - **15.1 逐項對照**：把本輪要處理的**全部**意見（含所有驗收方、含最新時刻的每一則
      ...
      走清法②裁定不修者核對「DECISIONS／PR 描述條目存在」、③證偽者核對「**證據或其可存取引用存在**」（僅有「已證偽」登記＝**缺項** （2026-08-17 更正，PR #53 第 5 輪））、⚪ 者核對「登記存在」；「diff」＝**已 commit 的差異**（HEAD 對 PR base，核對時點見 15.4 （2026-08-17 更正，PR #53 第 3 輪）），不是工作樹（此三類不產生 hunk，
      （2026-08-17 更正，PR #53 首輪）：原文「全部意見各有 hunk」會把合法清法變成違規）。
      **缺一項不得 push**＝清單存在「既無對應 hunk、也無②③⚪登記」的項目。
