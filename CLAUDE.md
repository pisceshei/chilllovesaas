...
15. **提交前復核鐵律（2026-08-17 使用者裁定；違反＝該輪作廢）**：
    **所有階段、所有 PR、所有驗收回應輪，push 到 GitHub 前必須做「逐項復核」**：
    - **15.1 逐項對照**：把本輪要處理的**全部**意見（所有驗收方、最新時刻的每一則判詞）
      ...
      ①**宣稱已修復**者 ⇒ `git diff origin/main...HEAD`（已提交差異，核對時點見 15.4）
      有對應 hunk；②**裁定不修**者 ⇒ DECISIONS／PR 描述條目存在；③**證偽**者 ⇒
      證據或其可存取引用存在（僅有「已證偽」登記＝缺項）；⚪ 者 ⇒ 登記存在。
      **缺一項不得 push**＝清單存在「既無對應 hunk、也無合規②③⚪登記」的項目。
    - **15.2 時序復查**：push 前**重新拉取 PR 最新留言**，兩種形式都拉——bot 判詞留言
      ＋Codex inline review（首次 push、PR 尚未存在時豁免；開 PR 後首輪意見照常進 15.1）。
