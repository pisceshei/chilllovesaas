...
15. **提交前復核鐵律（2026-08-17 使用者裁定；違反＝該輪作廢）**：
    **所有階段、所有 PR、所有驗收回應輪，push 到 GitHub 前必須做「逐項復核」**：
    - **15.1 逐項對照**：把本輪要處理的**全部**意見（所有驗收方、最新時刻的每一則判詞）
      ...
      基準 SHA＝作者每次 push 時自記（15.4 末步；判詞自帶「審 <sha>」時用作交叉驗證，
      **不依賴**判詞帶 SHA）。不用三點 `...`（rebase／force-push 後三點退回 merge-base，
      ...
      的 base ref 取代）僅作**初始交付盤點**，不作修復存在性證據；②**裁定不修**者 ⇒ DECISIONS／PR 描述條目存在；③**證偽**者 ⇒
    ...
    - **15.4 順序**：清單建立＋修復完成 → 全部閘門逐支**親眼看退出碼** → **commit** →
      ...
      push ＋ **自記本次 push 的 HEAD SHA**（倉庫外清單同處——下一輪 15.1 的比對基準）。**閘門後再動任何檔案＝回到本款起點重來**（重跑全部閘門、重新 commit，
