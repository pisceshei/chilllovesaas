...
# CHILL LOVE PoC — 平台層 tags（9 個；核心＝content_for 的 blocks 機制，見 24 §2.4、27 §6）
  ...
  module Tags
    ...
    class ContentFor < Liquid::Tag
      ...
      def render(context)
        ...
        frame   = context.registers[:frame] # 目前容器（section 或 block）的實例資料
        return "" unless runtime && frame

        case @kind
        when "blocks" # 依 block_order 渲染子 blocks
          order = frame["block_order"] || []
          order.map { |bid|
            bdata = (frame["blocks"] || {})[bid] or next ""
            runtime.render_block(bid, bdata, context)
          }.join
        when "block"  # 靜態 block：依 id 取實例資料；缺 → schema 預設（27 §6）
    ...
    # {% stylesheet %} / {% javascript %} → 匯集（PoC：吞掉，section 資產由 asset_url 載入）
    class Swallow < Liquid::Block
      def render(_context) = ""
    end

    # {% doc %}…{% enddoc %}（LiquidDoc；gem 5.8+ 才內建，PoC 自行提供）
    class DocTag < Liquid::Block
      def parse(tokens) # 內容不是合法 Liquid，逐 token 吞到 enddoc
    ...
    # {% form 'type', obj, attr: val %} → 最小 <form> 包裹
    class FormTag < Liquid::Block
      ...
      def render(context)
        inner = nil
        context.stack do
          context["form"] = ChillLove::FormDrop.new
          inner = super
        end
    ...
    # {% paginate expr by N %} → paginate drop 假分頁
    class PaginateTag < Liquid::Block
      ...
      def render(context)
        inner = nil
        context.stack do
          context["paginate"] = ChillLove::PaginateDrop.new(page_size: @size)
          inner = super
        end
        inner
      end
    ...
    # {% section 'name' %} → 靜態 section（設定存 settings_data.current.sections，27 §4）
