...
CASES = [
  ...
  [ "parity_path_forms", 1, "scripts/check parity.rb",
    "🔴 SCRIPT_REF「帶引號含空白」分支的承重斷言（第 3／5 輪指出零覆蓋）："     "把該分支砍掉，這支從 ci.yml 側消失 ⇒ 差集少它 ⇒ 本條轉綠被抓" ],
  [ "parity_path_forms", 1, "scripts/ci/check.rb",
    "🔴 「不帶引號含子目錄」分支的承重斷言（第 7 輪驗收指出原 fixture 零承重）："     "初版把這支放在 fixture **兩側**，而 SCRIPT_REF 是共用的——砍掉子目錄支援時"     "它從兩側**同時**消失 ⇒ 差集不變 ⇒ 突變存活。現在 ci.rb 側刻意不放它，"     "讓它成為**缺項**：砍掉子目錄支援 ⇒ 本 fixture 由紅轉綠 ⇒ 被抓。"     "🔴 教訓：**共用的解析器，fixture 必須讓目標只出現在一側**，否則解析器的"     "任何劣化都會對稱地作用在兩側、在差集裡互相抵銷" ],
