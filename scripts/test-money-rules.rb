...
# <!-- 🔴 2026-08-15 更正：本段原文寫「§K.7 的判準逐字是『只有前者綠不算交付』」，
#      那句話**不在 65 號裡**（`git grep "只有前者綠不算交付" docs/specs/` 零命中），
#      是轉述被當成原文再冠上「逐字」。語義沒錯，但「逐字」是一行 grep 就能證偽的斷言。
#      同一句話也曾被 PR #33 抄進 5 處，已於該 PR 一併更正。 -->
#
