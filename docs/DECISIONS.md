...
## D84（2026-09-05）全對齊授權與總目標：真店資料全權、admin／前台／編輯器逐字對齊、Ella 先行再 Minimog／Kalles 可裝、自行合併、每步 handoff

使用者裁定原文（`/goal`，2026-09-05）：「接下来我授权你使用所有的资料或者账户，包括shopify真店本尊里面的所有数据，这个shopify真店是用来测试，
不会用来真实使用。没有任何权限问题。你都可以任意新增，编辑，修改，删除等等。……所有的功能和ui都必须和shopify本尊一样，无论是商家admin后台还是
买家前台。……要逐字去对齐。功能必须100%的复制他的功能逻辑，业务逻辑。操作流程等等一切。Ui方面要精准到每一个字的字型，字体大小。模块的尺寸大小，
款式等等，ui交互逻辑。……必须结合shopify的官方说明文档，和shopify本尊的实际情况去交叉验证……没有任何版权问题。已经协商好。……先把ella这个主题的
所有功能完全复制到我们的项目，做到编辑器页面和买家前端所有页面例如首页，购物车，顧客帳戶登入和注册連結，商品详情页面等等一切的页面都完全一样。
第二部就是让其他我已经购买的主题例如minimog和Kalles可以直接安装在我们的店铺主题功能模块。……把平台管理后台，商家admin管理后台和买家前台全部对齐
起来。……中间我授权你自行合并，不需要等我人工合并。不需要找我申请任何权限。……你只需要每一个步骤做好handoff。要确保我可以在任何地方终止，然后
可以完美的交给另外的claude或者codex或者真人开发工程师进行接手。」（2026-09-05 第二次 `/goal` 追加終點句：「我们的最终目标是做出和shopify完全一样的saas系统才结束」——
工作不因單包完成而停止，直到三個面全部與本尊一致。）

- **授權（取代 D79 的「副本上操作」預設安全形）**：真店 pnrjnw-sy／hoko.vip 的全部資料與設定（商品、庫存、系列、市場、語言、顧客帳戶設定、主題
  含現行發布主題）可任意新增／編輯／刪除，用途＝取證與對表；仍守鐵律 12.2 唯一約束「避免產生真實費用與對外發信」，以及本檔 D83 的合併方式
  （`gh pr merge --squash --match-head-commit`、CI quality＋test 綠、18.3 清單亦代行）。不需再向使用者申請任何權限；需要**裁定**（兩種讀法會導向
  實質不同工作）時仍停下來問。
- **總目標與驗收**：三個面（平台管理後台、商家 admin、買家前台＋主題編輯器）與本尊逐字對齊——功能／業務邏輯／操作流程 100% 複製，UI 精確到字型、
  字級、模組尺寸、款式與互動；每項以「官方文檔 × 本尊實測交叉驗證」為取證（鐵律 12／16／19）；驗收若有任何不同即自動修到一致。
- **順序**：①Ella 主題全功能（編輯器頁面＋買家前台全部頁面：首頁、購物車、顧客帳戶登入／註冊連結與頁面、商品頁…）；②已購 Minimog／Kalles 可直接
  安裝、安裝後可裝修（平台契約全覆蓋，D82／theme-agnostic 目標）；③admin 與平台後台逐頁對齊；貫穿：每包考慮跨模組共用與 API 預留。
- **交接**：每個工作單位一份 handoff（鐵律 21，D47 入庫），詳細到任一時點可由另一位 Claude／Codex／工程師接手；路線圖＝
  `docs/plans/2026-09-05-全對齊路線圖.md`（本裁定同批建立，逐包更新狀態）。
- **落點**：本條；`docs/plans/2026-09-05-全對齊路線圖.md`；`HANDOFF.md` §0 指標；memory `session-handoff-pointer`／`full-automation-authorized`。
