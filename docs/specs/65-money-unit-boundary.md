...
## J. 與既有規格的衝突登記（本篇**不改**其他檔案，只登記）

| # | 衝突 | 現況 | 本篇立場 | 誰該改 |
|---|---|---|---|---|
| **M-1** 🔴 **P0** | `stripe_amount()` 的定義描述的是**裁定二之前**的儲存模型 | 15 §F4-5 逐字：「幣別：`stripe_amount()` helper 統一處理小數位（**JPY 等零小數幣別不乘 100**、個別幣別有整除規則）」 | 那句話在「JPY 儲存 1480」的舊模型下是對的。**裁定二之後 JPY 儲存 148000**，`stripe_amount()` 對 JPY 必須**除以 100**。照現行文字實作＝收款 100 倍 | **15 §F4-5** 改寫為引 65 §D；`stripe_amount()` 更名為 `to_psp_minor` 並改為值物件簽名 |
| **M-2** | 兩處指標指向已過時的定義 | 16 §F5「零小數幣別 → 只在序列化層由 15-F4 的 `stripe_amount()` 處理，業務層不感知」；55 §A「③零小數幣別 → 僅在序列化層由 15-F4 `stripe_amount()` 處理」 | **指標方向正確**（業務層不感知 ✅），但指向的定義已過時 ⇒ 改指 65 §D | 16 §F5、55 §A 各改一個交叉引用 |
