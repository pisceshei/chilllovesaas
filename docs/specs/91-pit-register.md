...
- doc-claims 歷史層警告（皆在 phase0 交接檔，位置以腳本輸出為準；exit 0、腳本管轄）——三輪重複登記，併一組【F11】
- phase0／session 交接檔以**本機路徑**作接手第一步與計畫出處（`~/.claude/projects/…`、`C:\Users\…`）——跨環境不可達（審核環境 Linux 實測）；接手指標的可攜性待收割輪定形【F5】
- §0.2 F1–F12 例證欄無檔:行錨（§1 尚空）——收割輪落地條目時補【F11】
- lint-prototype WARN 136（ERROR 0）——與本 PR 無關的既有原型債，B 軌處理【F1 存量】
- phase1 交接檔 §④ 單源句（歷史層）——更正註記已加（第 3–4 輪），結案候選【F5】
- 91 §0.1「擴類須在本表加列」指位含糊＋§0.2 無規約段——第 4 輪已修正文（指位改 §0.2、補擴類規約），結案候選【F6】
- #56 先合造成反向窗口（main 判詞模板已指 91 §3 而 91 當時只在本分支）——隨 #55 合併自動關閉，結案候選【F12】
...
> 名單複驗（**集合比對**，第 6 輪升級——原總數比對對「刪一補一」的一換一盲）：
> `git -c core.quotepath=false ls-files docs/worklog docs/handoff | sort | md5sum`
> 與 `grep -oE "docs/(worklog|handoff)/[^\`]+" docs/specs/91-pit-register.md | grep -v "：" | sort -u | md5sum`
> 相等（前者亦是 quotepath 旗標出處——中文檔名無此旗標輸出為八進位跳脫）。🔴 **新增 worklog/handoff 的同一 commit 必須同步
