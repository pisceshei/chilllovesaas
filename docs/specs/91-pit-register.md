...
> 積壓已轉入 §3（**來源範圍見 §3 標頭**——第 4 輪起指標句化，免第七份範圍句副本）。
...
當輪 worklog 記「為何既有 12 類不涵蓋」；不改既有代碼號語義。
...
> 不符，至少四處經摘寫/插錨/去腐）；**原文出處＝上列兩源**，展開 §1 引「檔:行」時
> 一律回讀原文；【F#】／候選標記＝轉入時另加的形態預標。
...
### 3.3 自 PR #55（本 PR）判詞 ⚪（第 1–3 輪，第 4 輪轉入；同根因併組）
