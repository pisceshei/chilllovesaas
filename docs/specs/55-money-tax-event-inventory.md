...
①折扣／稅的比例分攤 → **最大餘數法**（15-F2）；②百分比費用（重新上架費）→ **floor**（費用取小 ⇒ 退款取大，對買家有利）；③零小數幣別 → 業務層**完全不感知**，單位換算只發生在**跨界點**（R1 → 該 PSP pack 宣告的 `amount_format` 對應表示法，R5 或 R6），唯一出口是 `Money::Storage#to_psp_amount(psp:)`，契約見 **65 §D**（落地見 15-F4 第 5 點）。
🔴 **③其實不是捨入點**：`minor_units` 分支對不整除的餘數是 **raise（`NonIntegralPspConversion`），不是 round**（65 §D.2 A3；`decimal_string` 分支的對應是 A6 位數檢查，同一條紀律）。所以本表「只有三個捨入點」的斷言仍然成立，而且實際上只有兩個真的在捨入。
<!-- 依 65 §J M-9（69 §V-188）二次修正（2026-08-13），原文：「（R1 → 該 PSP pack 宣告的 minor unit），
     唯一出口是 Money::Storage#to_psp_minor(psp:)」「to_psp_minor 對不整除的餘數是 raise」。
     與 16 §F5.1 同形態：M-2 結案時的新名，同日被 69 §V-188 再改名（to_psp_amount，不留別名）而二度過時；
     M-9 登記表未涵蓋本處，2026-08-13 grep 補出。分錄禁以 R5/R6 記帳（65 §F.1）與 M02 冪等鍵恆 R1（65 §E.1-3）不變。 -->
