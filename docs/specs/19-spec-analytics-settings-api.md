...
1. 寫一份 `metrics.md` 指標辭典，每個指標三要素：公式、口徑、反例。起步集：
   - 🔴 **總銷售額（Total sales）＝ 銷售總額 − 折扣 − 撤銷款項 ＋ 稅額 ＋ 關稅 ＋ 運費 ＋ 費用**
     ...
     - **撤銷款項（`sales_reversals`）的組成見 `docs/specs/86` §3.2**——退款＋取消單全額＋
       訂單編輯的負向差額，**各自計入自己的發生日**。
...
| `Total sales` | ＝ Σ訂單總計 − Σ退款 **＋ Σ編輯增量**（三項各自帶自己的發生日） |
| `AOV` | ＝ `Total sales / Orders`；分母不含幽靈訂單 → **AOV 不被編輯污染**（這是本專案優於本尊的點） |
...
**必測**：①跨日編輯後 `Orders` 不變；②編輯增量落在編輯日、原訂單日的歷史數字不變（重跑 rollup 後仍不變）；③`Total sales` ＝ 三項相加；④`order_edit_deltas` 與 `orders.total_cents` 的變化量對帳零差異。
