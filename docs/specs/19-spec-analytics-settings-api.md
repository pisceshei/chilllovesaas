...
1. 寫一份 `metrics.md` 指標辭典，每個指標三要素：公式、口徑、反例。起步集：
   - 🔴 **總銷售額（Total sales）＝ 銷售總額 − 折扣 − 撤銷款項 ＋ 稅額 ＋ 關稅 ＋ 運費 ＋ 費用**
     ...
     - **撤銷款項（`sales_reversals`）的組成見 `docs/specs/86` §3.2**——退款＋取消單中
       **未走退款的部分**＋訂單編輯的負向差額，**各自計入自己的發生日**。
...
| `Total sales` | ＝ Σ訂單總計 − Σ**撤銷款項** **＋ Σ編輯增量（正向）**（各項帶自己的發生日）。撤銷款項的三個互斥組成見 86 §3.2——**編輯的負向差額已在撤銷款項裡**，本項只補正向增量，不得再減一次 |
