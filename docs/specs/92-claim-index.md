...
> 所有 `docs/specs/92-*` 之間的 CLAIM ID 必須全域唯一，否則 `scripts/check-doc-claims.rb` R6 阻擋。
...
- baseline: HEAD
- recheck: `ruby -EUTF-8:UTF-8 -e 'rows=File.readlines("docs/specs/71-admin-parity-sweep.md",chomp:true).grep(/^\| 71-R/); statuses=rows.map{|l|p=l.split("|",-1);[p[1].strip,p[-2].strip]}; containing=statuses.count{|_,s|s.include?("⬜")}; leading=statuses.count{|_,s|s.start_with?("⬜")}; mixed=statuses.select{|_,s|s.include?("⬜")&&!s.start_with?("⬜")}.map(&:first); derived=[containing,leading,mixed.size,mixed.first,containing,leading]; triage=File.readlines("docs/specs/84-m1-gate-triage.md",chomp:true).first(7); published_values=[triage[3].match(/\*\*(\d+) 列狀態格/)&.[](1)&.to_i,triage[4].match(/\*\*(\d+) 列以/)&.[](1)&.to_i,triage[4].match(/另 (\d+) 列/)&.[](1)&.to_i,triage[4].scan(/71-R\d+-[A-Z0-9]+/).first,triage[5].match(/這裡的 (\d+) 是/)&.[](1)&.to_i,triage[5].match(/未結案 (\d+)/)&.[](1)&.to_i]; abort "unexpected unresolved status split #{[containing,leading,mixed].inspect}" unless [containing,leading,mixed]==[77,76,["71-R3-DOC1"]]; abort "published split drift #{published_values.inspect} != #{derived.inspect}" unless published_values==derived; p({contains_open:containing,leading_open:leading,mixed:mixed,published_synced:true})'`
- status: corrected
