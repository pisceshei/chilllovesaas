...
- **只有三個捨入點**，其餘一律精確整數運算：
  ...
  3. **零小數幣別（JPY 等）** → 業務層**完全不感知**；單位換算只發生在**跨界點**（R1 儲存 cents → 該 PSP pack 宣告的 minor unit），唯一出口是 `Money::Storage#to_psp_minor(psp:)`，契約見 **65 §D**（落地見 15-F4 第 5 點）。
     🔴 注意這一點**不是捨入點**：`to_psp_minor` 對餘數的處置是 **raise，不是 round**（65 §D.2 A3）。JPY 的 `148050`（¥1,480.50）代表**上游算錯了**（湊整規則沒套用，29 §3.3），悄悄抹掉那 50 會讓對帳永遠差幾分錢卻查不出來。所以本節「只有三個捨入點」的斷言**不因本點而破**——第三點根本不捨入。
