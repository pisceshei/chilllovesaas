...
| **§9 新發現（新章）** | — | — | **11 項** | +11 |
| **列次口徑（未跨章去重）** | — | — | **180**（表 1–5 小計 162＋§6 七列＋§9 十一列） | — |

列次口徑複驗：`ruby -EUTF-8:UTF-8 -e 's=File.read("docs/specs/53-ui-gap-recheck.md"); p({section_6:s.scan(/^\| \*\*R-\d{2}\*\* \|/).size,section_9:s.scan(/^\| \*\*N-\d{2}\*\* \|/).size})'`；
162 是原四表加 §5 的既有小計，不再稱為全文件合計。180 只表示三段原始列次相加，
不是 distinct gap 總數；§6／§9 已知有 R-01/N-01、R-02/N-02、R-03/N-03、R-05/N-04、
R-06/N-06、R-07/N-07 六組成對重述，本檔不在未完成全文件去重前宣稱 distinct 總數。
