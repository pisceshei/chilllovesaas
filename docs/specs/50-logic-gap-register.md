...
**結論：表內 C-01～C-16 共 16 列，其中 C-01～C-15 是衝突，C-16 是「三方一致正面清單」
的正面對照，不計入缺口；衝突清單仍不完整——另遺漏至少 10 條，補列如下。**
...
**表 3 缺口數：`25`**（C-01～C-15 共 15 條衝突 ＋ T-01～T-10 共 10 條衝突；
C-16 是正面對照，不計入缺口）。複驗：
`ruby -EUTF-8:UTF-8 -e 's=File.read("docs/specs/50-logic-gap-register.md"); c=s.lines.grep(/^\| C-\d{2} /); positive=c.count{|l| l.include?("三方一致正面清單")}; conflicts=c.size-positive; t=s.scan(/^\| \*\*T-\d{2}\*\* \|/).size; abort "unexpected structure" unless [c.size,conflicts,positive,t]==[16,15,1,10]; p({c_rows:c.size,c_conflicts:conflicts,positive_controls:positive,t_conflicts:t,table_3:conflicts+t})'`。
...
### P1（來源 32 個分組；下游拆分 33 條，M4/M5 前修）

> ✅ **2026-08-12：33 條已全數處置**，修正紀錄與測試案例見 **`docs/specs/54-p1-logic-fixes.md`**。
> 分佈：**已修正 31**（其中 **14 條在 P0 輪已順帶修畢**，本輪僅複核確認並補完殘缺面）／**標待查證 1**（T-04 市場 `shipping` 繼承，V-02）／**降級關閉 1**（H-97 逾期請款 1.75% 附加費——本尊金流商定價，我方非收單機構，與 TW-9 電支條例鐵律衝突）。
> **本簿下列 4 條的原判定經驗證後不完全成立**，54 號已逐條記錄並縮小範圍：**H-112**（29:118 已有反方向敘述，非「29 與 15 都沒有」）、**H-106**（19-F4 已有 settings key 註冊表機制，非「無模型」）、**TW-10**（硬下限的處方會讓「合理例外情事」商品無法設定，改為軟 guard ＋ 法定管道恆存在）、**H-94**（help 實為 6 條、dev 實為 5 條，聯集 9 條，非「5+4」）。
> **口徑更正**：本節列出的來源分組實為 **32 個**，原標題的 33 來自 `H-101/H-103` 併寫；54 號按拆開的 33 條處理。複驗：`ruby -EUTF-8:UTF-8 -e 'a=File.read("docs/specs/50-logic-gap-register.md"); x=a[a.index(/^### P1/)...a.index(/^### P2/)]; rows=x.lines.select{|l| l.match?(/\A(?:\u91D1\u984D\/\u5951\u7D04\u985E|\u72C0\u614B\/\u5951\u7D04\u985E|\u898F\u5247\u985E)\uFF1A/)}; src=rows.join.scan(/(?:H|S|TW|T)-\d+(?:[\/\uFF5E](?:H|S|TW|T)-\d+)?/).size; dst=File.read("docs/specs/54-p1-logic-fixes.md").scan(/^#### P1-\d{2}/).size; p({source_groups:src,downstream_items:dst})'`。
> 54 號另列出本簿**漏掉的 8 條新問題**（NP1-A～NP1-H），其中 **NP1-D（`orderEditCommit` 未強制冪等 → 重複扣款/退款）** 與 **NP1-F（訂單編輯未觸發發票折讓/補開）** 屬金額與稅務正確性，已一併修掉。
...
| 表 3 · 三方衝突 | **25** | C-01～C-15 共 15 條衝突＋T-01～T-10 共 10 條；C-16 是正面對照 |
| 表 4 · API↔UI | **132** | 正向 32（孤兒 24＋TBD 8）＋反向 59（孤兒 16＋應刪/標 4＋28 缺 39）＋未結案 TBD 41 |
| 表 5 · 台灣落地 | **27** | 10 個主項下的可實作性缺口逐條計 |
| **合計** | **313** | 其中 **P0 15 條**、**P1 下游 33 條**、**必須查證 10 組**；五表加總複驗見 `docs/specs/92-claim-index.md` CLAIM-001 |
