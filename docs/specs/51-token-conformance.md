...
| **表 6** | 48 號自我宣稱查核 | 2 項宣稱 + §00 82 token（`e50b9120` 快照） | **宣稱二數字不實 + 衝突 11 條 + 命名分裂 1 項** |
...
>
> **表 6 快照邊界**：本表（含一覽列、§6.2 的 82／4／78、§6.4 的落地率與表末統計）
> 固定描述 commit `e50b9120cc9b2514fde4995a5ff4f6ff15332bff` 的 48／23／三份原型；這些值是
> 歷史稽核結果，不是目前 HEAD 的 token 數或落地率。若要發布目前樹的數字，必須另行重算。
...
> `e50b9120cc9b2514fde4995a5ff4f6ff15332bff` 的 48／23，不是目前 HEAD；82／4／78 只適用於
> 表 6 的這份歷史稽核，發布目前樹數字前必須重算。完整可重跑命令見
> `docs/specs/92-claim-index.md` CLAIM-005。
...
**§00 的 82 個 token 在原型中的落地率**（`e50b9120cc9b2514fde4995a5ff4f6ff15332bff` 快照）：
...
**表 6 統計（`e50b9120` 快照）**：宣稱一 **屬實**（9/9）；宣稱二 **數字不實**（實為 82 宣告 / 78 新增，非 63）；與 47 第三／四輪衝突 **11 條**（P0 4、P1 4、P2 1、已解除 2）；額外發現命名三方分裂 **1 項**。
