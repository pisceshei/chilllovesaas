...
| R2b | 全域 chrome 殘項（搜尋 CtrlK/快訊動態/Sidekick 形態/API 搜尋入口 MISS2/期間控制正式版 R1-V4/管道×期間全域下拉） | MISS2＋R1-V4 | ⬜ |
...
| 71-R1-V4 | V | 全域日期選擇器完整交互（雙月曆/對比期/粒度連動）為共用控件，本輪僅以 select 預設集近似——R2b 做正式版（實測已抓到本尊元件 S-SHOPIFYQL-DATE-CONTROLS 的完整選項集） | ⬜ R2b |
