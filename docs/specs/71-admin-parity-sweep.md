...
| G28 | inventory-idempotency-key-schema-required | 🔴 **庫存 mutation 的 `idempotencyKey` 上 GraphQL 契約層 `String!`（缺鍵＝GraphQL 驗證錯誤：HTTP **200**、`data: null`＋top-level `errors`——鐵律 4 三層中的第②層形態；🔴 本條原寫「非 200」是筆誤，2026-08-24 對抗審查抓到後更正）——對本尊的刻意加嚴**（使用者 2026-08-24 裁定，原文「idempotencyKey上string,批准進 71 §A」）。本尊形態＝`@idempotent` directive 的 runtime 檢查（2026-04 起強制但走業務錯誤層）；我方加嚴理由與 G-code 同構：admin SPA 是唯一客戶端，必填性推上 schema 讓前端**不可能**忘記傳，且錯誤分層裁定「必填性可推 schema、值域不可」（`docs/plans/2026-08-24-三道裁定-本尊對照.md` §四）。資料層對應：`inventory_adjustment_groups.idempotency_key` NOT NULL ＋ `(shop_id, idempotency_key)` 唯一索引（migration 20260824100000）。⚠️ 同案的 `changeFromQuantity required: :nullable` **未在本條內**——該欄位語義是「可顯式傳 null 跳過檢查」，其 schema 形態等第 17 包寫 mutation 時再定 |

| G29 | on-hand-adjust-no-ledger-document-ours | 🔴 **`on_hand` 調整免附 `ledgerDocumentUri`——對本尊的刻意放寬（ours）**（使用者 2026-08-24 裁定，全文＝`docs/DECISIONS.md` D45）。本尊語義「除 available 外全部必填」（95 §4）**不改**（外部事實照錄）；我方放寬理由：①我方模型 `on_hand` 由 `LEAF_COLUMN` 翻譯成 available leaf，要求它附一份 available 自己不准附的文件是自相矛盾 ②手動盤點沒有文件可附，必填的實效＝「這個入口不存在」（實測 100% 回 `INVALID_QUANTITY_DOCUMENT`）。其餘四個 name（reserved／damaged／safety_stock／quality_control）維持必填。守衛測試同時測兩邊（放行 on_hand ＋ 拒絕未附文件的 damaged），防止放寬被讀成全部放寬。**下一輪 parity 不得把本條「修回與本尊一致」**——要翻案須推翻 D45 |
| G30 | product-page-inventory-card-own-save-ours | 🔴 **商品頁庫存卡用卡內自己的儲存鈕，不掛頁面 SaveBar——對本尊的刻意差異（ours）**（PR #110 假設清單第 1 條，使用者合併即納）。本尊商品頁的庫存調整走頁面同一個 Save；我方分開的理由：商品頁 SaveBar 屬 `productSet` 那棵樹，把庫存併進去＝**一顆按鈕觸發兩條非原子的 API**，其中一條失敗時商家看不出哪一半存進去了。要合併需另開一輪（改 `InventoryCard` 與 `ProductDetailPage` 的 save handler，資料契約不動）。同 PR 的另一形態差異：歷程頁參考文件直接印 URI 字串（本尊是 popover）——資料已在手上、只差形態，屬待補不屬違規 |
...
7. **收尾**：worklog（三段制）＋本檔 §E/§F 更新與產物一起 commit／push；同一工作單位的
   handoff（四段制）另存 Git 倉庫外本地工作區，不 commit／push（D36／鐵律 21）。
...
**狀態圖例與計數**（2026-08-14 實數，`grep '^| 71-R' | awk -F'|' '{print $(NF-1)}'` 可複驗）：

| 標記 | 語義 | 數 |
|---|---|---|
| ✅ | 已結案 | 85 |
| ⬜ | **未結案** | **76** |
| 🚫 | **因裁定不適用**（非待辦，不得計入未結案） | 1 |
| | 合計 | 162 |

<!-- 2026-08-14 補。原本沒有圖例，於是 🚫 這個第三種狀態（目前只有 71-R13-V6，
     因 §A G26「不實作 POS」而不適用）被歷次交接文件當成未結案，
     「§F 未結案 77 條」這個數字**一路傳了好幾輪**，實際是 76。
     🔴 差一條不影響決策，但**登記簿的頭號數字沒有可複驗的定義**才是問題——
     多份文件引用它，卻沒有人能一眼確認它怎麼算出來的。圖例補上後可用上面那行 grep 複驗。 -->
...
| 71-R8-V4 | V | 2026 系列建立式＝「來源」卡（新增條件/新增商品同卡混用＋排除 negative 條件＋多來源組）vs 我方手動/智慧二分不可互轉（13-F4）——概念差待裁定 | ✅ **DECISIONS D13 已裁定採 sources 模型**（84 §2 B-4 同時作廢）。2026-08-24 實機複驗（`docs/research/94` §1）：Sources 卡存在、來源型別下拉四項、排序九值。🔴 但 **UI 四型 ≠ 資料模型四型**——API 上只有兩個具體 source 型別，products/variants 是同一型的 `targetType`、app 是欄位；且 include/exclude 是 source **內部**兩區塊、排除**逐 source 結算**、巢狀深度上限 **1**。我方 13 §F4 與 limits.yml 抄的是 UI 形態，五處待修（`docs/plans/2026-08-24-三道裁定-本尊對照.md` §1.2） |
...
| 71-R11-V12 | V | 🔴 **總銷售額可為負**（撤銷 > 銷售的日子，官方明列）——我方金額元件與 badge 必須支援負值顯示（tabular-nums 對齊），M1 前確認 | ✅ **2026-08-14 查證＋機制化**：schema 複驗 `unsigned` **0 個**、CHECK 約束 **0 個**——建表面本來就是對的。但沒有東西擋住未來有人加，因此在 `scripts/check-tenant-isolation.rb` 補**規則 3**：金額欄（`*_cents`）宣告 unsigned 即 CI fail，錯誤訊息寫明「總銷售額可為負，且在正常銷售的測試資料上測不出來」。正反皆測（exit 0／1） |
...
| 71-R13-V4 | V | **發布模型三層 AND** 有四個掛載點要同步：商品頁的上架管道區塊／目錄（R10）／市場（R10）／代理式目錄。help 原文：商品必須**同時**「在該管道市場指派的目錄內」**且**「已發布到該管道」才會上架。另：新增管道時既有商品**預設全開**；排程發布需商品為 Active、不支援單一 variant、Shop 管道不支援；`publicationCreate/Update` 單次上限 50 個商品 | ✅ **2026-08-14 落地： ＋ migration ＋ model ＋ 測試**——建 `publications` 與 `resource_publications`（多型：Product/Collection/ProductVariant），🔴 **移除 `products.published_at`**（同一件事不留兩個事實來源，鐵律 7 形態），既有值搬進關聯。本尊兩條排程限制已寫成 validation 並有測試（不支援排程的管道／不得為單一 variant 排程）。**第三層 catalog 只留欄位待 M5**——它是讀取時的過濾，加它不需改這兩張表結構（88 §3.2） |
