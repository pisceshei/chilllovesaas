...
- **不自動填、但要度量**：後台商品列表與 SEO 健康頁顯示「缺 alt 的媒體數」。若提供 AI 產生 alt，寫入時必須落 `alt_source: ai|human|imported` 稽核欄（🔴 D48 之後該欄隨 alt 一起在**檔案層**＝`files.alt_source`；已建於第 3 包遷移 `20260826055500`，值域正典＝limits `media.alt_sources`，後台人工寫入由 `StoredFile#stamp_alt_source` 自動標 `human`）——理由同 30 §1.2 的 scaled content abuse 防線：無標記的大量自動內容日後無法回溯清理。
...
| S3 | 檔案 `alt` 一級欄位＋`alt_source` 稽核＋缺 alt 計數（🔴 D48：權威在檔案層，原文寫 `media.*`） | **M1** | `files.alt_text`, `files.alt_source`（皆已建；後者隨第 3 包遷移） | 商品列表顯示缺 alt 數 |
