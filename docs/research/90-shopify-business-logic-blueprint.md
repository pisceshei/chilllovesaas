...
|---|---|---|
| **M0** | ① outbox 與業務資料同一 transaction（違反即 fail）② `webhook_id` 併發雙投恰一次處理 ③ `metafieldsSet` ≤25 atomic ＋ `compareDigest` 樂觀鎖：digest 不符**拒寫而非覆蓋** ④ 同 (app, shop) 併發 bulk query ≤5，超出以 `userErrors` 拒收**而非排隊** ⑤ GraphQL cost 預扣→結算→退還差額 | 🔴 **fixture pack 全套**：`minor_units`（Stripe／Adyen／Datatrans）、`decimal_string`（PayPal）與 `decimal_number`（Airwallex）**各至少一個 pack**<!-- 2026-08-31 更正：原文兩格式且 Airwallex 掛 decimal_string——已依 65 §D.4 更正為三格式 -->；JPY／TWD／KRW 三幣矩陣；**TWD 測「整除 100 違反 ⇒ raise 且不得自動湊整」**（docs/specs/65 §H.1／T16／T19）；`Money::Decimal` 誤傳 PSP adapter ⇒ `TypeError`（**在 HKD 上也會錯**，是基準法域測得到卻沒人在測的形態） |
