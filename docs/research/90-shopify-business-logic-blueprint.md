...
| S3 | 提交付款資訊 | buyer | 送 PSP authorize 或 authorize+capture | `open`（不變） | **我方：無變化**（本尊此刻 hold 庫存，我方不做 hold，§03 F.2#5） | OrderTransaction ∅ → `PENDING`（kind＝`SALE` 或 `AUTHORIZATION`，§05 B.2） | 無（尚無 order） | 3DS ⇒ `AWAITING_RESPONSE`（§05 A.4）；拒付 ⇒ 47 值 errorCode 之一（§05 A.5），回付款步驟重試 |
...
| 同一 transaction 內**必須一起發生** | 依據 |
|---|---|
| 行鎖重讀 `captured_total_cents` / `refunded_total_cents`，**條件式 UPDATE** 檢查 `Σrefunds ≤ captured − refunded`（禁止先 SELECT 再 INSERT） | §06 C.2 |
...
| M-11 | `line_tax_cents = bankers_round(unit_price_cents × tax_bp / 10000) × quantity`（**先單件捨入再乘量**，行小計法為錯） | §10 C |
