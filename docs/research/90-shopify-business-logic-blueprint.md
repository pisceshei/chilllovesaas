...
| S5 | 付款結果非同步落地 | PSP／reconcile job | 收 PSP 回呼或退避輪詢收斂交易狀態 | Order 不變 | 無 | `PENDING → SUCCESS/FAILURE/ERROR`；`AWAITING_RESPONSE` 超時 ⇒ 先查 PSP，查無 ⇒ `FAILURE`＋`PAYMENT_PROVIDER_ERROR`（§05 B.1.1 R1）；`UNKNOWN` 非終態、退避輪詢收斂（R2） | `order_transactions/create`（🔴 **update 也走這個 topic**，§04 E.1） | 逾 pending 過期日（官方口徑「典型約一週」）⇒ `EXPIRED`；**pending 期間官方明文鎖單**：不可編修品項／折扣／地址、不可 restock、不可取消、不可收款、不可 mark as paid（§05 B.3） |
...
| A-3 | 🔴 **AOV 是鐵律 7 的官方具名例外**：`AOV = (gross_sales_excl_adjustments − discounts_excl_adjustments) / orders` ⇒ `AOV ≠ net_sales/orders` 且 `AOV × orders ≠ total_sales`。**反向斷言必須入測試** | §14 C／§06 C／鐵律 7 註 |
...
| **前置未決** | §7 Q-93（MetaobjectStatus enum 是否恰為 DRAFT/ACTIVE）、§7 Q-93（metaobject 定義數 128 vs 256 兩源矛盾）、§7 Q-86（試用期長度，**使用者裁定**）、§7 Q-86（frozen→deactivated 期限，**使用者裁定**）、§7 Q-94（首發 24 topic 確切清單）、§7 Q-02（訂閱 owner 欄位 shop_id vs app_id，**使用者裁定；改表成本高，M0 必答**）、§7 Q-03（訂閱 schema 是否在 M0 遷移檔預留，**使用者裁定**） |
