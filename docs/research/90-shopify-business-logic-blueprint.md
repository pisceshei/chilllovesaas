...
|---|---|---|---|---|---|---|---|---|
| S0 | 加入購物車 | buyer | `add.js`：庫存粗檢；行合併鍵＝variant＋properties＋selling_plan＋價格四者全同才併行（§03 C.1） | Cart ∅ → `active` | **無**（cart 階段永不動庫存，§03 C.7-3） | 無 | `carts/create`／`carts/update`（僅線上商店 cart，§03 E.1） | 售罄 ⇒ 不加入回 422；不足 ⇒ 加到可用上限＋422（§03 C.2）。🔴 `update.js` **不驗**已在 cart 內品項的數量（§03 C.2），server 端不得信任 cart 數量 |
...
| S5 | 付款結果非同步落地 | PSP／reconcile job | 收 PSP 回呼或退避輪詢收斂交易狀態；🔴 結清**跨越付清門檻**時，同一 transaction 重算訂單投影（`displayFinancialStatus` `PENDING → PAID`）並發 `orders/paid`＋`orders/updated`（§5.1 承諾的事件） <!-- 2026-08-17 更正（PR #52 Codex 第 2 輪）：原寫「Order 不變、僅發 order_transactions/create」——付款依賴的履約/通知/分析會永遠不知道訂單已付清 --> | 未付清：Order 不變；付清：`displayFinancialStatus → PAID` | 無 | `PENDING → SUCCESS/FAILURE/ERROR`；`AWAITING_RESPONSE` 超時 ⇒ 先查 PSP，查無 ⇒ `FAILURE`＋`PAYMENT_PROVIDER_ERROR`（§05 B.1.1 R1）；`UNKNOWN` 非終態、退避輪詢收斂（R2） | `order_transactions/create`（🔴 **update 也走這個 topic**，§04 E.1） | 逾 pending 過期日（官方口徑「典型約一週」）⇒ `EXPIRED`；**pending 期間官方明文鎖單**：不可編修品項／折扣／地址、不可 restock、不可取消、不可收款、不可 mark as paid（§05 B.3） |
...
| S9 | 運送事件流 | carrier／app | `fulfillmentEventCreate`（11 值，§09 B.4）；3PL 開 tracking 支援時平台每小時拉單號 | `displayStatus` 由**最新一筆事件**推導（18 值計算欄，不落 DB）；首次 `IN_TRANSIT` 落 `inTransitAt` | 無 | 無 | `fulfillment_events/create`／`delete` | ⚠️ 官方**未定義事件間合法順序**，`DELAYED`／`ATTEMPTED_DELIVERY` 可穿插（§09 B.4）；local delivery 無 carrier 事件、不觸發 out-for-delivery／delivered 通知（§09 B.7） |
...
| **S13** | 🔴 **退款（原子點 T3）** | staff | 算 suggested（floor 0）→ 建 Refund＋RefundLineItem＋pending REFUND 交易＋restock＋稅務事件＋分析回沖（見 2.2 T3） | Refund ∅ → 存在（🔴 **無 status 欄**，§06 B.4）；Return 全處理且全 restock ⇒ 自動 `CLOSED`（§06 B.1 T5） | 依 `restockType` 4 值（§06 A.4）：`CANCEL`（未履行 ⇒ 自訂單移除＋`committed −`／`available +`）｜`RETURN`（已履行 ⇒ `on_hand +`／`available +`）｜`NO_RESTOCK`｜`LEGACY_RESTOCK` 建立時 reject | 建 `REFUND`(parent＝capture/sale)；`PAID` → `PARTIALLY_REFUNDED` → `REFUNDED`（終態）。上限＝`captured − refunded`（**軟上限**，見 2.4 M7） | `refunds/create`（🔴 **在金流確認前即發**，官方語義「independent from the movement of money」，§06 E.1）、`order_transactions/create` | PSP 退款失敗 ⇒ 本地 pending 交易列＋告警＋**同一把 idempotency key** 重試（§06 D.3-5）；退款**不可撤銷**（§06 B.4） |
| S14 | 訂單封存 | 系統／staff | 自動封存（「已付款且已履行」或「已全額退款」）或 `orderClose` | Order `open` → `closed`（`closedAt`）；`cancelled` 與 `closed` 可疊加（§04 B.1） | 無 | 無 | `orders/updated` | 建立退貨會**自動解除封存**（§04 B.1 互鎖）；`closed` 判定式＝所有品項已履行或已取消 **AND** 所有金流交易完成（兩條件合取） |
...
| 庫存：**同一 location** `committed −q`、`on_hand −q`＋ledger 同 transaction | §02 B.1／D.1-3 |
...
| 同一 transaction 內**必須一起發生** | 依據 |
|---|---|
| 行鎖重讀 `captured_total_cents` / `refunded_total_cents`，**條件式 UPDATE** 檢查 `Σrefunds ≤ captured − refunded + approved_over_refund_cents`（`approved_over_refund_cents` 預設 0，僅在 `allowOverRefunding=true`＋`orders.over_refund` 權限＋二次確認通過後由授權分支寫入——與 §4 R-2/R-3、§2.4 M7 對齊；禁止先 SELECT 再 INSERT） <!-- 2026-08-17 更正（PR #52 Codex）：原式無條件用 captured − refunded，會把 M7 明載合法的 over-refund（已退 store credit 改退原卡）擋死，與自家 R-2 矛盾 --> | §06 C.2 |
...
| restock：依 restockType 動庫存＋ledger（冪等 key＝`refund_line_item` id，webhook 重放不重複進貨） | §06 D.3-4 |
...
| A4 | buyer／staff | 顧客走 invoice checkout 付款／admin 標記已付／`draftOrderComplete`／付款條款完成 | 🔴 **T1 的變體**：同 transaction 建 Order（**另取**訂單序列新號）＋`reserved → committed` 原子遷移＋`draft.status=COMPLETED`＋`completedAt`／`order` 回填＋metafields 單向複製 |
...
| B3 | staff | `orderEditCommit(notifyCustomer, staffNote)` | 🔴 同 transaction：品項差異落地＋FO line items 同步＋庫存增減（加品 ⇒ `available −`／`committed +`；減品 ⇒ 依 restock 決策回補）＋outbox |
...
| D6 | 系統 | 自動關閉 | 條件＝**每個品項都已處理且每個退貨品項都已 restock** ⇒ Return → `CLOSED`（可 `returnReopen` 回 `OPEN`） |
...
| C11 | `OrderTransaction.kind` 付款鏈 | 8 | AUTHORIZATION／CAPTURE／SALE／VOID／REFUND／CHANGE／EMV_AUTHORIZATION／SUGGESTED_REFUND | AUTHORIZATION 或 SALE | VOID／REFUND／CHANGE | **append-only 事件鏈**（kind 建立後不可改），狀態靠新增子交易推進 | §05 B.2 |
...
| F6 | `Market.status` | 3 | DRAFT／ACTIVE／DELETED | DRAFT | DELETED（硬刪不可復原） | 持久化 enum（本尊 enum 僅 2 值＋硬刪） | §11 B |
...
| `fulfillmentCreate` | FO 累加 `fulfilled_quantity`；庫存 `committed−N`／`on_hand−N`，`available` 不變；訂單履行軸重物化 | 連動 | §09 E／§02 B.1 |
...
| `returnProcess` 處理完全部品項**且全部退貨品項已 restock** | Return 自動 OPEN→CLOSED | 連動 | §06 E |
| RFO line item disposition = RESTOCKED | 庫存依指定 location 回補；冪等 key＝`refund_line_item` 唯一鍵 | 連動 | §06／§02 |
| `refundCreate` 成功 | 產生 `OrderTransaction(kind=REFUND)`；Order 金流軸重算為 PARTIALLY_REFUNDED／REFUNDED | 連動 | §06 B.4／§05 B.3 |
...
| M-11 | `line_tax_cents = bankers_round(taxable_unit_cents × tax_bp / 10000) × quantity`（**先單件捨入再乘量**，行小計法為錯）；`taxable_unit_cents`＝行項**分攤折扣後**金額的單件基數（M-9 的 taxable_base ÷ quantity；分攤不整除 ⇒ 依 D-12① 最大餘數法定各件次基數） <!-- 2026-08-17 更正（PR #52 Codex）：原式用未扣折扣的 unit_price_cents，與緊鄰 M-9「稅基＝折扣後」矛盾，照抄會對折扣品多收稅 --> | §10 C |
...
| S-14 | 同一 order 的全部 FulfillmentOrder（含 cancel 替代單、split 兩半、move 產物、replacement FO）對每個 line_item 的 quantity 總和 **≡ 該訂單該品項的可履約數量** | §09 C |
...
| P-8 | `AWAITING_RESPONSE` 超時**先查 PSP**：查得終局照落；PSP 無終局則轉 FAILURE ＋ `PAYMENT_PROVIDER_ERROR`，**禁止轉 UNKNOWN**（兩態語義不得混用）⚠️ 本尊行為官方全無明文 | §05 B.1.1-R1 |
...
| V-2 | 某管道可得(product) ＝ `status=ACTIVE` **∧** 已發布到該 publication **∧** 在指派給該管道市場的 catalog 內（三層 AND，缺一不可得） | §01 C／§11 |
...
| V-18 | UrlRedirect **只在 resolve 判定為 404 時生效**；path 仍能渲染出頁面則靜默不生效且不報錯（UI 須主動提示） | §12 C |
...
| A-3 | 🔴 **AOV 是鐵律 7 的官方具名例外**：`AOV = (gross_sales_excl_adjustments − discounts_excl_adjustments) / orders` ⇒ `AOV ≠ net_sales/orders` 且 `AOV × orders ≠ total_sales`。**反向斷言必須入測試——但 fixture 必須含 post-order adjustments（至少一筆 reversal）並斷言實際公式** <!-- 2026-08-17 更正（PR #52 Codex 第 2 輪）：無調整日兩式數值上恰好相等，無條件不等式斷言會把合法 rollup 打紅 --> | §14 C／§06 C／鐵律 7 註 |
...
| A-20 | 所有輸入陣列 items ≤ 250、connection `first/last` ≤ 250；連線分頁越過 25,000 物件後 count 一律**封頂回報 25,001**（不得回真值） | §15 C／鐵律 4 |
...
| X-6 | active automatic 折扣數併發撞 25 上限 | 建立/啟用兩路徑各自計數 | 鎖或條件寫入；超出回 `ACTIVE_PERIOD_OVERLAP` | 併發建立第 25、26 個 ⇒ 恰 1 個超額被拒 | §07 C |
...
| X-8 | restock 重複進貨（webhook 重放／退款重試） | 退款可重試但庫存調整非冪等 | 冪等 key ＝ `refund_line_item` 唯一鍵，同一 line item 只產生一次 inventory adjustment | 重放同一 `refunds/create` 3 次 ⇒ 庫存只 +N 一次 | §06 C |
...
| `returns/close` \| `returns/reopen` | 全品項處理完自動關閉／`returnClose`／`returnReopen` | 06 | 04, 13, 14 | ✅ | 自動關閉條件含「全部退貨品項已 restock」 |
...
| # | 生產 → 消費 | 事件 | 契約與失敗處理 | 來源 |
|---|---|---|---|---|
| A1 | 04 / 09 / 06 → 15 通知 | 訂單確認、出貨通知、可取貨通知、退貨核准·拒絕、@mention | 通知信一律在 transaction 外寄；渲染時**回查資源現值**而非用 payload 快照 | §09 E、§13 D.6 |
...
| D-47 | 金流 | `AWAITING_RESPONSE` 超時與 `UNKNOWN` 收斂官方均無明文 | 自訂 R1–R3：超時先查 PSP，查無終局轉 FAILURE＋`PAYMENT_PROVIDER_ERROR`（**禁轉 UNKNOWN**）；UNKNOWN 指數退避輪詢；逾放棄期限 alert 轉人工，資金狀態不得永久懸置 | 加嚴 | §05 B.1.1（→Q-21） |
...
| D-74 | 稅務 | 正向計稅＝banker's rounding、**單價粒度**（unit→round→×qty） | 照抄並落鍵 `tax.rounding: bankers_unit_then_quantity`（粒度已定，禁改回含混的 line_level）；與退款折讓的 floor 差額法**不是同一算法** | 照抄 | §10 F#4 |
...
| D-85 | 前台 | redirect 僅在原 URL 回 404 時生效，來源仍可渲染則靜默不生效 | 照抄；額外在 UI 主動提示「來源仍可訪問則不生效」 | 照抄 | §12 F |
...
| `plans.merchant_metaobject_definitions` | §15 docs 頁 128 vs 2025-10-20 changelog 256 | 暫以 **changelog 256**，待 Q-93 |
...
| **退款上限** | **M4** | 兩個並發退款不得合計突破 `captured − refunded`；DB 唯一硬約束僅 `refunded_total_cents >= 0`（§06 invariants） |
