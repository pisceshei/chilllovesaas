...
**快取**：成功回應快取 **15 分鐘**、錯誤 **30 秒**；cache key＝**完整正規化費率請求的 canonical digest**——即上方請求 payload 的全部計價欄位：variant IDs＋數量＋箱規／重量＋品項單價（**折扣調整後** `price`）＋`properties`＋`rate.currency`＋`rate.locale`＋carrier service ID＋origin＋destination（（2026-08-17 更正，PR #52 第 18 輪）：原鍵漏 價格/幣別/locale/properties——carrier app 可依其中任一欄計價，鍵撞則錯額/錯幣/錯服務的報價被快取重用 15 分鐘；凡送給 app 的計價欄位皆入鍵，新增欄位隨 digest 自動生效）。
