...
4. 副作用：restock 走庫存調整（冪等 key＝refund_line_item 唯一）；`returns/process`＋`refunds/create`＋`reverse_fulfillment_orders/dispose` webhook；**全部品項處理完且全部 restock ⇒ Return 自動 CLOSED**。
