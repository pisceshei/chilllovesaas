...
- 官方數例：42 件 × $14.99、18% 稅。單件稅 $2.6982 → 捨入 $2.70；42 × $2.70 = **$113.40**。舊 invoice 層算法：42×14.99=629.58×0.18=113.3244 → **$113.32**。兩法相差 $0.08，**line 層捨入是官方現行**。
  🔴 **捨入粒度已定（2026-08-14 補證，原 openQuestion 關閉）＝「單價×率 → banker's rounding → ×量」**。官方對同一數例的分步敘述（轉述）：先把**單件**稅 $2.6982 捨入成 $2.70，再乘以 42 件得到行稅 $113.40。「行小計×率再捨入」（629.58×0.18=113.3244→$113.32）被此分步**排除**。官方措辭「at the line-item level」指「捨入發生在行內、而非 invoice/訂單層」；行內實際粒度＝單件。公式（我方表示法，rate 用 basis points，見 F.2#10）：`line_tax_cents = bankers_round(unit_price_cents × tax_bp / 10_000) × quantity`。
