...
3. 退款／取消（帶 refund）／訂單編輯（負向）／退貨 → `SalesFactLine(kind=reversal, occurred_on=處理日)`。互斥拆分依 86 §3.2（refunds 擁有「真的動了的錢」全部，其餘只補未走退款的部分；86 管**來源互斥**，金額段口徑＝商品段——C.1 拆分量，86 §3.2 已同步（2026-08-17 更正，PR #52 第 12 輪））。
