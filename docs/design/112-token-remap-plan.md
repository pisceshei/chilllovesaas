...
### 2.1b 語義色（`--sem-*`）✅ **已完成（D56，2026-08-28）**

換值前 **77 顆**（5 族 × 5 層 × 3 態 ＋ 2 顆按鈕例外），其中**只有 22 顆被消費、55 顆是死 token**。

| 類 | 顆數 | 處置 |
|---|---:|---|
| 有本尊對應、值不同 | **52** | 逐格採用本尊 live computed 值 |
| 有本尊對應、值已相同 | **0** | — |
| **本尊無此槽 ＋ 我方零消費** | **25** | **刪除**（D54：不得保留自創值） |
...
只換 fill ⇒ **3/5 族直接 AA 失敗**。成對換 ⇒ 5/5 通過，最低 5.20。

**fill 亮度分布**：換值前 0.166–0.177（跨度 **0.010**）→ 換值後 0.125–0.779（跨度 **0.654**，65 倍）。
那個窄帶是自動產生器的指紋——把每個色相壓暗到白字剛好過 4.5 就停。
