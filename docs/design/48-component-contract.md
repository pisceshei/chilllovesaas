...
3. **單位是 `em` 不是 `px`**。`em` 在 media query 條件式中以**初始字級 16px** 計算，不受 `html{font-size}` 影響，所以 px→em 的換算是安全的。
   （🔴 2026-08-28 D63 實測複驗，不再只是規格推論：本機 Chrome、同源 iframe 控寬，
   root 宣告字級依序設 8/13/16/26/32px，`(min-width:48em)` **一律在 768px 翻轉**，
   與 `(min-width:768px)` 同格。注意它證的是「**root 宣告**字級不影響」；
   「使用者改**瀏覽器預設**字級會不會移斷點」本輪**未實測**——而它會移，
   正是 47 §F 選 em 的理由。）
...
### 34.8 第三個變體：`cl-locale-switch--storefront`（前台語言切換器，2026-08-13 新增）

**來源**：67 §F.2（切換器的內容與形態）、67 §A.5(b)（白名單是切換器的**唯一**直接消費者）、67 §F.1(b)（前綴恆帶地區）、62 §K.2（切換器必須是真實連結）。
**原型落地狀況**：`docs/design/chilllove-storefront-v2.html` — `sfLocaleLinks()`（產生器，頁尾與抽屜共用）／`sfOpenLocales()`（值域）／`sfGoto()`／`setLocale()`；DOM 錨點 `#fLocaleSw`（頁尾常駐）與抽屜內 `data-doc="locpicker"`。CSS 類 `.loc-sw`。

| | 與 `--ui` / `--content` 兩個 admin 變體的差別 |
|---|---|
| 值域 | 🔴 **`open_locales(當前 market)` ∩ `published`**——不是 `shop_locales` 全集。裁定逐字：「前台不用全部顯示出來」 |
| 形態 | 🔴 **真實 `<a href>` 清單**，不是 `<select>`、不是純 JS。爬蟲靠它發現其他語言版本 |
| 每個選項 | `endonym`＋**它會產生的 URL 前綴**（`/en-ca`）；`hreflang` 與 `lang` 屬性各掛一個 |
| 目前項 | `aria-current="true"`（不是 `aria-checked`——這是連結不是 radio） |
| 切換後 | **換 URL**，不是改一個全域變數 |

🔴 **三條硬規則，每一條都對應一個具體事故形態**：

1. **只列 `③ ∩ published`。** 這讓它**在定義上不可能**產生 404 連結——它與路由表（`UNIQUE (shop_id, domain_id, url_prefix)`）是同一個來源。站內任何一條 404 的語言連結都是 bug，掛 lint（形態同 67 §F.4 的 `routes` lint）。反過來說：**不得**做成「列全部語言、再把不能點的擋掉」——那會產生一批看得到卻打不開的選項，而商家會以為是壞了。
2. **切語言＝換 URL。** 改全域變數而不換 URL ⇒ 同一條 URL 對不同人回不同語言 ⇒ 爬蟲索引到隨機語言版本、hreflang 的自指與雙向性（62 §I.1 不變量 1／2）失去意義。驗收 `SF-2`：對同一 URL 送三種 `Accept-Language`，回應主體必須逐位元組相同。
3. **與地區切換器是兩個控件，不得合成一個**（`i18n.market_locales.switcher_is_two_controls: true`）。換語言只換文字；換地區換的是幣別、稅、運費、可售性、法域。合成一個 ⇒ 買家想看英文，結果連幣別一起變了。恆帶地區的前綴（`en-hk`）字面上就是「語言＋地區」，很容易被實作成「一個下拉列出所有前綴」——🔴 **前綴的形態是 URL 的事實，不是 UI 的規格**。允許的形態是兩個控件放在同一個 `<form>`／同一個彈出層（對應 `{% form 'localization' %}` 的 `country_code` 與 `language_code` 兩個欄位名）。

**邊界情況**（沿用 §34.6，兩處差異）：
- **只有一個開放語言**：整個元件**不渲染**（與 §34.6 同一條）。原型的美國市場就是這個狀態，可直接對照。
- **切地區後當前語言不在新市場的開放集**：落到新市場的 `default_locale`（29 §4，本輪補上「開放集」這個限定詞），並在 toast 說明落點。**不得**保持原語言——那會導向一條不存在的前綴（404）。

**兩處尚未實作**（給正式版的契約，不是原型現況）：`{% form 'localization' %}` 的實際 POST（原型直接改 hash）、以及建議橫幅（`Accept-Language` 只決定要不要顯示橫幅與預設高亮，**不得**改變同一 URL 的輸出內容）。
...
## §37 市場開放語言清單 `cl-marketlocales`（2026-08-13 新增）

**來源**：67 §C.8(c)（admin 契約，本節逐條落地）、67 §A.5(a)（三個集合）／§A.5(b)（呈現決策 vs 存取控制）／§A.5(d)（只能加不能減 ＋ 兩個分區）、67 §C.8(b)（每個 presence 恰一個預設 locale）／§C.8(d)（寫入的連帶動作）、62 §I.2（hreflang 碼）／67 §F.1(b)（URL 前綴）、29 §1.5(d)（繼承分區的既有形態，**沿用不新造**）、48 §32（排序列）／§11（badge）／§16（關閉確認對話）。
**原型落地狀況**：`docs/design/chilllove-admin-v2.html` — `mktLocaleCard()`／`mktOpenLocales()`／`mktDefaultLocale()`／`mktPrefixFor()`／`urlPrefixOf()`／`hreflangCodesOf()`／`mktLocAddOpen()`／`mktLocAddOK()`／`mktLocClose()`／`mktLocDefault()`／`mktLocMove()`／`mktI18nInvalidate()`；DOM 錨點 `data-doc="mk-locales"`、列 `data-doc="mk-loc-row"`。
- **已實作**：§37.1–§37.4、§37.6 的三個數字確認對話、預設 locale 不可關閉、兩個分區、值域只收 published、多國市場的「N 個碼」展開。
- **尚未實作**（給正式版的契約，不是原型現況）：真正的 HTML5 拖曳（原型用 ↑／↓ 兩顆按鈕，鍵盤路徑因此天然可用，見 §37.4）、`closed_at` 的實際落庫與「曾經開過」的歷史查詢、多 presence 市場（原型每個市場最多一個自有 presence）。

> 🔴 **這個元件存在的唯一理由，是把「啟用一個語言」和「把它開給某個市場的買家」變成兩個動作。**
> 合成一個的後果（裁定明文）：商家一啟用日文，**全世界所有市場**的切換器立刻多一個日文選項。
> 而把它做成「刪掉譯文來關掉語言」的後果更貴：關掉一個市場的日文＝刪掉整批日文譯文，想再開就要重翻。
> **譯文是資產，曝光是決策——這個元件就是那條分界線的 UI 形態。**

### 37.1 解剖

```
cl-marketlocales                     卡片（§13），位置＝**市場設定頁的「網域與語言」分區**
│                                    🔴 不另開一頁：放到「設定 → 語言」那頁商家會找不到，
│                                       因為那頁的心智模型是全店（§C.8(c) 明文）
├─ header                            標題 ＋ `N / max`（引 limits.i18n.max_shop_locales）
├─ p.cl-card__lede                   🔴 一句話講清楚「這是呈現決策不是存取控制」
├─ cl-banner--caution（多國市場才有）  逐國展開成 N 個碼 ＋ subfolderSuffix 不是國碼的警語
├─ section「本市場開放」（可增刪、可排序）
│  └─ cl-marketlocales__row × N      見 §37.2
├─ section「繼承自 {父市場}」（唯讀）   形態沿用 29 §1.5(d) 的繼承徽章，**不新造一套**
│  └─ cl-marketlocales__row × N      無排序、無關閉、只有「到 {父市場} 調整 ›」
└─ footer                            「開放語言」按鈕 ＋ 「商店語言設定 ›」 ＋ 值域說明
```

🔴 **兩個分區不是版面偏好，是 §A.5(d) 的可執行形態**：缺了分區，商家會在子市場頁找不到刪除按鈕，然後去客服說「壞了」。有了分區，「這一項是父市場給的、要改去父市場改」這件事在畫面上就是自明的。

### 37.2 單列 `cl-marketlocales__row` 的欄位（五樣，缺一個就少一個商家看得懂的後果）

| 欄位 | 內容 | 為什麼一定要有 |
|---|---|---|
| `endonym` ＋ 標籤 | `繁體中文` ＋ `zh-Hant` | 商家認語言靠自稱，工程對帳靠標籤 |
| **URL 前綴** | `/zh-hant-hk` | 這一列**會產生一條 URL**，不寫出來商家不知道自己按下去多了什麼 |
| **hreflang 碼** | `zh-Hant-HK`；多國市場顯示「**N 個碼**」可展開 | 62 §I.3(d)：條目數是商家唯一能看懂的「這個決定有多大」的指標。27 國市場一次多 27 行 `<head>` |
| **翻譯進度徽章** | 讀 `translation_status`（§11 badge） | 🔴 與語言設定頁的百分比、商品列表徽章、翻譯後台**同源**（鐵律 7）。任一處自己 `GROUP BY` 就是下一個對不上的數字 |
| **未發布警示** | 「此語言未發布 ⇒ 這一格在前台是 404、不進 hreflang／sitemap」 | §A.5(c) 情形 4。這是**唯一**會讓「設定看起來對、前台卻是 404」的組合 |

🔴 **前綴與碼的來源不同，這一點在列上就要對**：URL 前綴取**presence 擁有者**（繼承來的語言走父市場那條 URL），hreflang 碼取**本市場**（它的地區才是要宣告的那個）。兩邊都取本市場 ⇒ 子市場憑空多出一條與父市場逐字相同的前綴，`UNIQUE (shop_id, domain_id, url_prefix)` 被違反；兩邊都取父市場 ⇒ 子市場的地區永遠不會出現在標註裡。

### 37.3 完整態表

| 態 | 列 | 動作區 |
|---|---|---|
| default（本市場開放） | 底 --surface | 進度徽章 ／ 預設 radio ／ ↑ ↓ ／ 關閉 |
| default（繼承） | 底 --surface | 進度徽章 ／ `cl-badge--info`「繼承自 {父市場}」 ／ 「到 {父市場} 調整 ›」 |
| 是本市場預設語言 | 副標多一段「**本市場預設語言**」 | radio 已選；**關閉鈕仍在但點下去回 userErrors**（見 §37.6） |
| 該語言未發布 | 副標多一段紅字警示 | 動作不變——🔴 **不得把列 disabled**：它是合法設定，只是目前被閘門擋住，disabled 會讓商家以為要去別的地方改 |
| 第一列／最後一列 | — | ↑ 或 ↓ `disabled`（§A.1：真 disabled，不是 aria-disabled——它沒有需要解釋的理由） |
| 市場沒有任何 presence | 整個「本市場開放」分區換成 `cl-banner--caution` | 「`inherit_primary`：不產生新的 URL 前綴」＋ 「開放語言」按鈕 |

### 37.4 鍵盤與焦點

- 排序用 **↑／↓ 兩顆按鈕**，不是只有拖曳。§32.4 說得很清楚：拖曳排序的鍵盤路徑是這一類元件**最容易做錯的一節**；先做按鈕版等於鍵盤路徑天然可用，日後補拖曳時把它當成 §32 的 fallback 即可。
- 每顆按鈕都有 `aria-label` 帶語言名（「把 English 上移一位」），否則螢幕閱讀器只會聽到一串「按鈕 按鈕 按鈕」。
- 預設語言是 `radio`（單選，恆為開放），不是 toggle——toggle 表達不了「恰一個」。
- 排序或改預設之後以 `aria-live` 播報新順序／新預設；🔴 播報要說出**後果**（「這會改變本市場自動重導的落點」），不是只說「已更新」。

### 37.5 響應式

- ≤767：每列改為兩行（第一行語言＋徽章、第二行前綴＋碼＋動作），前綴與碼**不得截斷**——它們是這個元件的資訊主體。
- 多國市場的「N 個碼」在窄版預設收合，展開後縱向列出。

### 37.6 邊界情況（本節是這個元件最貴的部分）

- 🔴 **關閉語言的確認對話必須顯示三個數字**：①將 404 的 URL 數（該市場 × 該語言 × 可索引資源數）②將從 hreflang 移除的條目數 ③**該語言在該市場的既有譯文筆數（不會被刪，只是不再曝光）**。第三個專門用來回答商家的「我的翻譯會不會不見」。形態與理由同 67 §E.6(a) 的匯入預覽——**破壞性操作在按下確認前必須有一個數字**。
- 🔴 **開啟語言不需要確認對話**，但要顯示「將新增 N 個可索引 URL」（形態同 62 §J.3）。
- 🔴 **不能關閉預設 locale**：`userErrors{code: MARKET_DEFAULT_LOCALE_CANNOT_BE_CLOSED}`，訊息要求商家**先改預設再關**。關掉預設語言 ⇒ 地區重導沒有落點（62 §K.2）、`{% form 'localization' %}` 切國家後沒有語言可落（29 §4）。
- 🔴 **值域只收 `published = true` 的語言**：未發布語言**不出現在新增選單裡**，但要在選單裡寫出「有 N 種未發布，先去發布」——不寫的話商家會以為那些語言消失了。
- 🔴 **子市場不得減父市場的語言**：繼承分區沒有關閉鈕，而且要**寫出替代做法**（「不要把那個語言開在父市場，改為在需要它的每個子市場各自開」）。一個做不到它字面意思的開關，比沒有這個開關更糟。
- **從 ② 移除語言時**：③ 必須同步清掉該語言（三層真包含 ③ ⊆ ② ⊆ ①），且**移除與關閉是兩件事**——移除刪譯文，關閉不刪。兩者的確認文案必須各自寫清楚，不得共用。

### 37.7 實作備註

- 🔴 **粒度是 web presence，不是 market**（§C.8(a) 明文要求寫進程式碼註釋）：一個市場可以有多個 resolved presence（自身 ∪ 祖先），所以「市場的開放語言」是**聯集的結果**，不是一個可以直接 UPDATE 的欄位。任何 `UPDATE market_locales WHERE market_id = ?` 形態的寫入都是 bug。
- 🔴 **不新建 `market_locales` 表**（`i18n.market_locales.separate_table_forbidden: true`）：概念名與 API 名用 `market_locales`，實體沿用既有的 `market_web_presence_locales` ＋ 補三欄（`is_market_default`／`open_to_buyers`／`closed_at`）。新建表的後果是同一件事兩張表、而且**兩張表的粒度不同**（market vs presence）。
- 🔴 **`open_to_buyers` 是欄位不是「刪除該列」**：刪列會讓 `position` 與 `closed_at` 一起消失，而「這個組合曾經開過」是決定 404 範圍與失效範圍的必要資訊。**關閉是狀態轉換，不是刪除。**
- 🔴 **FK 指向 `shop_locales` 而不是 `platform_locales`**：這一條 FK 就是「③ ⊆ ②」的執法點。指錯目標的話，商家可以把一個**沒有啟用**的語言開給市場，前台切換器上就會出現一個沒有任何譯文、也沒有 published 狀態的語言。
- **寫入的連帶動作四件，同一個 transaction**（§C.8(d)）：①bump `shop_locales_version`（切換器出現在每一頁）②觸發 hreflang ＋ sitemap 失效管線（去抖 5 分鐘）③關閉時寫 `closed_at` 並把該 (market, locale) 路由標記為 404 ④🔴 **不刪任何 `translations` 列**。
- **class 名對照（原型 → 正式版）**：`set-row→cl-marketlocales__row`、`sr-t→cl-marketlocales__label`、`sub→cl-marketlocales__meta`、`sr-act→cl-marketlocales__actions`、`tr-b→cl-badge`。
...
| 34 | 語言切換器（介面／內容／**前台**三變體） | `cl-locale-switch` | **67 §E.1（兩層語言＝兩個切換器，不得連動）／§E.2／§C.6（進度徽章同源）／§F.2＋§A.5(b)（前台變體：只列市場開放集、必須是真實 `<a href>`、與地區切換器是兩個控件）**、64 §3/§4、47 §H2-2 |
...
| 37 | 市場開放語言清單 | `cl-marketlocales` | **67 §C.8(c)（admin 契約）／§A.5(a)(b)(d)（三個集合・呈現決策・只能加不能減）／§C.8(b)(d)（預設 locale 與連帶動作）**、62 §I.2／67 §F.1(b)（碼與前綴）、29 §1.5(d)（繼承分區，沿用不新造）、48 §32／§11／§16 |
