...
> 計畫列：`docs/plans/2026-08-24-三方向執行順序.md` 第 7 列 · 67 §L 的 **L4**（依賴 L3，L3 已在 M1 落地）
...
| `resources:` | `[Product \| Collection]` | 必填 | 上限 `i18n.resolve.max_resources_per_batch`（50），超過 raise |
...
| `depth` | 0..n+1 | 0＝請求語言命中；1..n＝第 n 階 fallback；n+1＝落到 base |
...
1. **請求語言＝來源語言時 `depth` 必須是 0**。來源語言的文字在 base row，`translations`
   按定義一列都沒有（`Upsert` 用 `SOURCE_LOCALE_NOT_TRANSLATABLE` 擋）。照「查不到就一路
...
4. 🔴 **讀寫共用同一份實作，讀取端不做簡化版快篩**（ours）。研究階段建議讀取熱路徑只跑
   不可見字元快篩（~1.1µs）、完整判準（~85µs）留給寫入端；**否決**——四個可翻欄位裡
   只有 `body_html` 是 `:html`，一張商品詳情頁 ≈1 次（85µs），50 筆列表頁全解析也只有 ~4ms，
   而代價是「同一個值讀寫兩種答案」的漂移，正是本模組存在的理由。

`CONTENT_BEARING` 來源＝WHATWG「Embedded content」分類
（<https://html.spec.whatwg.org/multipage/dom.html>，2026-08-25）＋`hr`／`table`／表單控件三項加項。
...
CLDR `common/supplemental/parentLocales.xml` 把 `zh-Hant` 的 parent 寫成 `und`（＝root），
官方理由逐字：`If the locale is written in the non-default script for the language, it needs
to have the parent specified as root, for example, zh_Hant and sr_Latn need to fall back to
root (not to zh and sr which are in different scripts).`
...
順序＝**sanitize → 判空 → 量長度**，三者不可對調（理由見 `Upsert#sanitize` 的行內註釋）。
...
1. **`translations` 必須進 `limits.catalog_flow.cache_stamp_sources`**——67 §G.3 明文
   ...
   第一次渲染翻譯欄位時 raise。
   🔴 **本包不加這個值**：現行 `cache_stamp_sources` 的每一項都指向**實際存在的欄位**，
   加一個指向不存在欄位的來源名＝把假設寫成事實（鐵律 19）。**第 3 包立欄時一起加。**
2. **粒度已裁定**：`i18n.cache.translations_stamp_granularity: resource_locale`（已在 limits）。
3. **載體建議（U21）**：借 `translation_status.updated_at`——它的唯一索引恰為
   `(shop_id, resource_type, resource_id, locale_tag)`，零新欄。
   🔴 **前提是 §4.2 的缺口已補**，本包已補。
4. **`shop_locales_version`（U22）**：全店級、無欄、無寫入面、無裁定。建議放 `shops`，
...
| 編號 | 內容 |
|---|---|
| **P7-L1** | 🔴 **截尾鏈在今天的生產資料上不可達**。`shop_locales belongs_to :platform_locale`，而現行 `platform_locales` 的 28 列裡沒有任何一列的截尾目標也在表內（`pt-BR`／`pt-PT` 的父 `pt` 不在表內；`zh-Hant`／`zh-Hans` 的父 `zh` 是禁用碼）。複驗＝`bin/rails runner "puts PlatformLocale.pluck(:tag).inspect"`。⇒ §C.4 步驟 2 目前只在單元測試（自行補平台字典列）中生效。補平台字典是 **ML-0** 的事，不是本包 |
| **P7-L2**（＝U1） | 🔴 **我方的 fallback 鏈與本尊不同構**，屬 ours 分歧，**不得寫成「對齊本尊」**。本尊 Markets fallback 的官方範例全用敘述名稱（`Base Dutch`），第 3–4 階是**同一 market 的另一語言**而非 BCP-47 截尾。取得方法：鐵律 12 在測試店 `chill-love-u5q5mnzq` 實測 |
| **P7-L3**（＝U2） | ⚠️ **本輪研究最大的誤讀陷阱**：本尊 checkout/customer-account **extension locale 檔案**確實有五階截尾鏈（`de-DE → de → fr-FR → fr → default`，官方明文），但那是 **extension JSON 翻譯檔**，**不是** `translatableResource` 的商家內容譯文。兩者是不同子系統，不得互相引用 |
| **P7-L4**（＝U8） | 本尊 `translationsRegister` 對「送空字串」的實際行為、以及本尊是否把**語義空 HTML** 也視為未翻譯並回落，官方文檔只標 `value: String!` 與 `TranslationErrorCode.BLANK` ⇒ **未取得**。不得從型別反推行為 |
| **P7-L5**（＝U7） | 本尊是否對回落內容加註 `lang` ⇒ **未取得**。Liquid 物件面已確認無此能力（`shop_locale` 只有 `iso_code`／`endonym_name`／`name`／`primary`／`root_url`）⇒ 我方 `mark_fallback_with_lang_attribute_on` 是 **ours 加嚴** |
| **P7-L6**（＝U14） | 我方 MySQL 實例的 `@@eq_range_index_dive_limit`／`@@range_optimizer_max_mem_size` **現值尚未實查**；`max_resources_per_batch: 50` 的推導用的是官方預設值 200。上線前跑 `SELECT @@eq_range_index_dive_limit, @@range_optimizer_max_mem_size;` 並落 worklog |
| **P7-L7**（＝U9） | RFC 4647 §3.4.1 的**多 range language priority list** 與 `*` 跳過規則未實作。目前 `resolve` 只收單一 locale ⇒ 不適用；**接 `Accept-Language` 前必須補**（外層要跑遍所有 range 才 fallback 到 source，不得對每個 range 各自 fallback 一次） |
| **P7-L8**（＝U19） | `field_key` 到 67 §B.1 三分類的**權威對照倉庫內不存在**。`Fields::MISSING` 是我方裁定；`Upsert::REQUIRED_FIELDS` 是**進度分母**，語義不同。兩者目前值相同純屬 v1 射程小，`spec/services/translations/fields_spec.rb` 有 tripwire |
| **P7-L9**（＝U20） | Resolve 對 `PRODUCT`／`COLLECTION` 以外 resource_type（`SHOP_POLICY`／`PAGE`／`METAFIELD`／`THEME_LOCALE_CONTENT`）的射程未定。`Translation::RESOURCE_TYPES` 目前封閉在兩值 |
| **P7-L10**（＝U3／U4／U5） | `digest` 的正規化實作本尊未公開；我方既有 `Translation.digest_for` 與 `limits.i18n.digest_normalization` 不相符，`Upsert#severity_for` 也與 67 §C.5(b) 的字元層編輯距離不同。**既有落差，不在本包射程**（鐵律 20.5）。讀取端**不得**自行重算 digest 當判準，只讀 `translations.outdated` 欄 |
...
- 可靠的繁簡誤借判別需要繁簡字表。唯一成熟的公開字表是 OpenCC 的
...
- ⚠️ 另有 **U15 未取得**：OpenCC 詞庫對電商領域詞（加入購物車／結帳／運費／退款／庫存）
  的覆蓋率未知（`TWPhrases` 只有 825 條且以資訊科技與一般詞彙為主）⇒ 引入後實際能抓到多少
  仍待量測。
- ⚠️ 另有 **U18 未取得**：字形啟發式的誤判率是在 CLDR `exemplarCity`（838 個平均 3.6 字的
...
突變驗證（18 格，每一格都必須讓對應 spec 轉紅）＝
`docs/worklog/2026-08-25-第7包譯文解析.md` 的突變表。
