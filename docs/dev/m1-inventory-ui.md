...
## 出貨後對抗式複查修掉的（2026-08-24）

第 18 包合併後跑了一輪六視角對抗式複查，修掉的要害有三個，都**不是**單一地點的
demo 店測得出來的：

1. 🔴 **換地點必須丟棄 pending**。pending 的 `compareAgainst` 是「在舊地點看到的值」，
   而 save 送出時帶的是**當下的** `locationId` ⇒ 用 A 倉的 CAS 基準寫進 B 倉。
   兩倉恰好同值時 **CAS 還會通過**，庫存就這樣進了錯的地點且沒有任何錯誤訊息。
   現在換地點一律清 pending 並出 toast（靜默丟棄同樣不誠實）。
2. 🔴 **歷程連結必須帶 `locationId`**。歷程是 **(品項, 地點)** 的帳，不是品項的帳；
   不帶時後端退回 priority 序第一個地點，多地點商店會看到別的倉庫的歷程。
3. 🔴 **`on_hand` 免附 `ledgerDocumentUri`**（D45）。原本照本尊語義必填，
   導致 On hand 行內調整 100% 失敗。詳見下節。

另修：卡的歷程連結原本永遠指 `rows[0]`（多變體必指錯）、save 會清掉儲存期間新 stage 的格子、
`createdBy` 的 N+1、頁量硬編碼、缺 keyset 索引、載入失敗偽裝成「沒有庫存」、保留天數硬編。

### 🔴 與本尊的第三處刻意差異：on_hand 免附文件（D45）

本尊語義是「除 `available` 外 `ledgerDocumentUri` 全部必填」（95 §4）。我方對 `on_hand`
放寬，理由兩條：①我方模型裡 `on_hand` 由 `LEAF_COLUMN` 翻譯成 `available` leaf，
要求它附一份 available 自己不准附的文件是自相矛盾；②手動盤點沒有文件可附。
其餘四個 name 維持必填，且**有測試同時守住兩邊**（放行 on_hand ／ 拒絕未附文件的 damaged）——
只測放行那半，這個放寬會慢慢被讀成「全部放寬」。

### 兩個工程坑（都讓 spec 紅了才發現，寫在這裡免得再踩）

- **`.squish` 的 SQL 不能用 `--` 行內註釋**：squish 把換行壓成空白，
  `--` 之後的 `WHERE`／`ORDER BY`／`LIMIT` 全部變成註釋。查詢照樣成功，只是語義全變。
  要註在 SQL 裡只能用 `/* */`，或寫在 heredoc 外面。
- **`private` 必須在所有 GraphQL field 解析方法之後**：插在中間會讓後面的欄位
  在執行期「不存在」。
...
- 🔴 **跨租戶授權洞（既存、非本包引入）**：`authorize_inventory!` 只認平台級 `owner` 布林，
  不驗該員工屬於當前 shop；`Current#can_access_shop?` 方法存在但零呼叫點。
  已裁定另開資安專包修（見 `docs/worklog/2026-08-24-庫存介面對抗式複查修復.md` Pending 第 1 條）。

## 變更記錄

- 2026-08-24：建立（排程第 18 包，PR #110）
- 2026-08-24：出貨後對抗式複查的修復（換地點 pending／歷程 locationId／D45 on_hand
  放寬／N+1／keyset 索引／頁量引 limits 等 12 項）
