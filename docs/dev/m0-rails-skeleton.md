...
商品列表以 `(created_at, id)` keyset cursor 分頁，不使用深 OFFSET。前端目前固定 `first: 50`，server 端以 `config/limits.yml` 限制單頁最多 250 筆。空資料時顯示「還沒有商品」與「新增商品」CTA；新增／編輯路由在 M0 仍是 placeholder，CRUD 屬 M1。
...
- Money API 後續以 `MoneyV2`／`MoneyBag` 序列化；資料庫內仍只存 integer cents + currency。
...
- `spec/migrations/m0_core_schema_spec.rb`：47 個具名表、額外冪等表、`shop_id` 第一欄、tenant-prefixed indexes、composite FK、integer cents。
- `spec/config/m0_configuration_spec.rb`：品牌單一來源、§9.4 配額與 GraphQL 邊界。
