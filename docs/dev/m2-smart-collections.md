...
- 🔴 `Rebuild.call` 的回傳有**兩種** `:skipped`：`error: nil`＝**該系列不是智慧系列**
  （手動成員不歸本服務管；2026-08-26 第七輪 L5 更正——第三輪 G1 起早退判準已從
  「沒有 conditions source」改成「非智慧系列」，零 source 的智慧系列**照樣**走掃尾，
  那正是 G1 修法的核心）；`error: LOCK_TIMEOUT_ERROR`＝鎖等逾時、這一輪沒重建。**消費者必須分辨**
...
**P11-B10（2026-08-26；經 G4→H5→J3→L1 四次修正後的最終形）**：exclusion 的 `collection`
型讓系列 A 的成員資格依賴系列 B 的物化列。`ResyncProduct` 依引用關係做**拓樸排序**
（被引用者先算）⇒ **單向引用**（含 A→B→C 鏈）在一次事件內收斂。
🔴 **環（任何長度，含自引）自 2026-08-26 第七輪（L1）起在寫入層一律拒收**：
「A 排除 B」讀 B 的**物化成員** ⇒ 反單調函數 a := ¬b。奇數環是奇數次反單調的合成，
**沒有不動點** ⇒ 成員週期震盪；配上第六輪的反向傳播（K8，以「有沒有變」為傳播條件）
就是**無界 job 鏈＋無界 outbox**（實測 n=3／n=5 永不終止，n=2／n=4 各 1／2 個 job 就停）。
判準不是「偶數放行奇數拒收」——偶數環雖會停，答案卻取決於起始狀態 ⇒ 環在這個語義下
沒有一個「對」的答案，一律拒。
⚠️ **守衛上線前建立的既有環**仍可能存在：`ReferenceGraph` 的 `visiting` 標記讓排序不
無窮迴圈，反向傳播帶 **visited 集合**（不是深度上限——深度上限擋的是**路徑長度**，
會把合法的無環長鏈截斷，2026-08-26 第八輪 M1）⇒ 環走一圈就止步、無環長鏈照走完，
鏈長天然有界於「這家店的智慧系列數」。既有環的成員答案仍不保證正確——這是本項殘留
的登記面。
🔴 **並行建環**（兩個請求各加環的一端）由 `replace_sources!` 內的**店級序列化複查**
擋下（第八輪 M2）：`normalize` 的 pre-flight 是純讀、在 txn 與列鎖之外，而列鎖只鎖
被編輯的那一列 ⇒ 構造上不互斥，必須在序列化點內再驗一次（沿用 `HandleChange` 的
店級鎖慣例）。
...
**P11-B13（2026-08-26 第十一輪；🔴 交由第 13 包裁定）**：`collection` 型 exclusion
（A 排除 B 時讀 B 的**物化成員**，95 §1.3 的六種 exclusion 型之一）是本包幾乎全部
併發機制的來源——拓樸排序、反向傳播、環拒收、店級序列化、以及它們各自引發的
死鎖與回歸。它在 v1 **沒有任何前端入口**（`rg -n "referencedCollectionId" app/frontend`
零命中；規則編輯器＝第 13 包）⇒ 商家在 v1 完全用不到它。
⇒ 第 13 包開工前先裁定：**續作**（則需連同本包的引用圖機制一起驗收）或
**改設計**（例如改成「重算 B 的規則」而非「讀 B 的物化列」，那樣就沒有跨系列的
順序與傳播問題）。在裁定之前，本包已交付的實作維持現狀、不再擴張。
...
🔴 **這張表存在的理由**：本包經多輪對抗審查，確認的 finding 逐條列在
`docs/worklog/2026-08-25-第11包智慧系列引擎.md` 的各個「整合修復」節（數量請數該檔，
不在此寫死——2026-08-26 第七輪 L9 更正：原文寫「三輪 10 條」，早已過期）。
其中**絕大多數是同一形態**
