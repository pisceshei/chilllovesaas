...
> 收斂機制現為**五項**：原四機制（判定層）＋鐵律 15 提交前復核（交付完整性層，見文末段；2026-08-17）。
...
把「自動驗收永遠找得到新問題、因此永遠不通過」這個結構性問題修掉。五件事：
...
4. `MAX_FIX_ROUNDS` 由「換一句留言」升級成**真閘門**
...
**突變測試 6/6 全抓**：R1 停用／裸檔名分支拿掉／R3 停用／R4 停用／錨定變全放行／canary 拿掉。
...
## 第五機制：鐵律 15 提交前復核（2026-08-17，PR #53）

流程＝修復 → 閘門 → commit → **逐項核對已提交差異**（回應輪對**上輪 push 的 HEAD** 兩點 diff、SHA 由 push 時自記；對 PR base 的累計 diff 僅作初始盤點——同 CLAUDE 15.1）（宣稱已修復者有
hunk；清法② 與 ⚪ 核對登記存在、③ 核對證據存在）→ **重拉兩種留言**（bot 判詞＋
Codex inline；首推豁免）→ push。與前四機制的關係：四機制管「判定怎麼下」，
