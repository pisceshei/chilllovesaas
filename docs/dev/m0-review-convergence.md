...
- 判定規則是二元的：**任一條 🔴 即需修改**，沒有嚴重度門檻。
  <!-- 2026-08-17 起收斂機制新增**鐵律 15 提交前復核**（PR #53）：修復→閘門→commit→
     逐項核對 git diff base...HEAD（宣稱已修復者有 hunk；清法②⚪ 核對登記、③核對證據）→
     重拉兩種留言（bot 判詞＋Codex inline；首推豁免）→push。目前為紀律條款、
     機制化（腳本比對判詞清單 vs diff）待辦——與既有四機制互補：四機制管「判定」，
     鐵律 15 管「回應輪的交付完整性」。 -->
