...
> **接手順序：總方案 → 最新 `docs/worklog/` → 操作者提供的倉庫外本地 handoff（若有）→
> `CLAUDE.md` 全部現行鐵律**。`docs/handoff/` 自 D36 起是歷史唯讀資料，不再作現況入口；
> 本地 handoff 不存在於 clone 時，不得把歷史 handoff 外推成現況，改以 Git／PR 實物重新取證。
