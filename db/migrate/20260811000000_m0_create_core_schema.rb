...
class M0CreateCoreSchema < ActiveRecord::Migration[8.1]
  ...
  # 法域 schema 聯集表（docs/research/06 §7.1，依 56 §0.2 原則 3／57 §G-04）：
  # schema 一律取**所有 jurisdiction pack 的聯集**，行為才取當前 pack。
  # HK 首發時 einvoices／einvoice_allowances 恆空，但表必須現在建——
  # 🔴 einvoices 的「不得建 (shop_id, order_id) 唯一索引」是全案唯一
  # schema 級、上線後改不得的決定（一訂單多發票，55 §D G-04）。
  JURISDICTION_UNION_TABLES = %i[
    einvoices einvoice_allowances jurisdiction_capability_skips
    contract_liability_entries
  ].freeze

  TENANT_TABLES = ((DOCUMENTED_TABLES - [ :shops ]) + [ :idempotency_keys ] +
                   JURISDICTION_UNION_TABLES).freeze
  ...
  TENANT_RELATIONSHIPS = [
    ...
    %i[einvoices orders order_id],
    %i[einvoice_allowances einvoices einvoice_id]
  ].freeze
  ...
  def create_shops
    create_table :shops, comment: "租戶根；依規格明確豁免 shop_id" do |t|
      ...
      # 預設值依 2026-08-12 使用者裁定「基準法域＝香港」（CLAUDE.md 鐵律 11）：
      # HKD／Asia/Hong_Kong。原骨架（2026-08-11）為台灣預設（TWD＋台北時區）——
      # 早於該裁定，2026-08-13 移植時整檔修正（含全部 14 個 currency 欄位預設）。
      # （本註釋刻意不寫舊時區字面量：spec 以「字面量不得出現」做防回退斷言。）
      t.string :store_currency, null: false, default: "HKD", limit: 3
      t.string :timezone, null: false, default: "Asia/Hong_Kong", limit: 64
  ...
  def create_catalog_tables
    ...
    create_tenant_table :product_variants, comment: "商品變體與 integer-cents 定價" do |t|
      ...
      t.bigint :cost_cents
      t.string :currency, null: false, default: "HKD", limit: 3
      t.integer :position, null: false
  ...
  def create_commerce_tables
    create_tenant_table :checkouts, comment: "結帳快照及棄單來源" do |t|
      ...
      t.string :presentment_currency, null: false, default: "HKD", limit: 3
    ...
    create_tenant_table :orders, comment: "訂單金額快照與彼此獨立的狀態機" do |t|
      ...
      # 🔴 一筆交易有兩個法域（56 §A.0；06 §7.1）：賣方決定稅制／憑證／儲值監管，
      # 買方決定隱私／取貨／幣別／消費者權利。訂單成立即快照
      # （limits.jurisdiction.snapshot_on_order），混成一個 country 欄位跨境單必錯。
      # 無 default——法域快照必須由建單流程明確寫入，預設值＝靜默錯法域。
      t.string :seller_jurisdiction, null: false, limit: 8
      t.string :buyer_jurisdiction, null: false, limit: 8
      t.integer :lock_version, null: false, default: 0
    end
  ...
  # 建立法域 schema 聯集表（docs/research/06 §7.1；56 §0.2 原則 3）。
  #
  # 為什麼現在建：HK 首發時 einvoices 一列資料都不會有，建表的人沒有任何
  # 動機去想索引怎麼建；等日後啟用 tw pack 才發現索引建錯，就要停機
  # migration（06 §7.1 逐字）。核心流程只發「稅務事件」，pack 決定要不要
  # 落地成憑證（CLAUDE.md 鐵律 11）。
  #
  # @return [void]
  # @see docs/research/06-data-model.md §7.1
  # @see docs/specs/56-jurisdiction-architecture.md §A.3、§B.3.1
  # @see docs/specs/57-p0-hk-baseline-fixes.md §G-04、§G-07
  def create_jurisdiction_tables
    create_tenant_table :einvoices, comment: "政府稅務憑證（HK 恆空；tw pack 用）" do |t|
      t.bigint :order_id, null: false
      t.string :jurisdiction, null: false, limit: 8
      t.string :status, null: false, default: "pending", limit: 32
    ...
    # 🔴 一訂單多發票（55 §D G-04；limits.jurisdiction.schema_union_rules.
    # forbidden_unique_indexes）：**不得**對 (shop_id, order_id) 建唯一索引。
    # 這是全案唯一 schema 級、上線後改不得的決定——改動本行前先讀 06 §7.1。
    tenant_index :einvoices, :order_id
    tenant_index :einvoices, %i[jurisdiction status]

    create_tenant_table :einvoice_allowances, comment: "發票折讓（HK 恆空；tw pack 用）" do |t|
      t.bigint :einvoice_id, null: false
      t.bigint :amount_cents, null: false
      t.string :currency, null: false, default: "HKD", limit: 3
      t.string :reason, limit: 255
      # Σ amount_cents(per einvoice_id) ≤ einvoices.total_cents 由應用層
      # 條件式 UPDATE 保證（55 §D G-02，tw only）——DB 層無法表達跨列聚合約束。
    end
    tenant_index :einvoice_allowances, :einvoice_id

    create_tenant_table :jurisdiction_capability_skips, comment: "法域能力 documented_no_op 記錄（HK 首發即大量寫入）" do |t|
      t.string :jurisdiction, null: false, limit: 8
      t.string :capability, null: false, limit: 64
      t.string :event_kind, null: false, limit: 64
      t.string :source_write_point, null: false, limit: 128
      t.string :reason, null: false, limit: 255
      t.datetime :occurred_at, null: false
      # 為什麼是表不是 log（56 §A.3）：讓「什麼都沒做」變成看得見的一列資料，
      # nightly 斷言才有受測物——HK 下 20 條稅務事件全部走這條路（56 §B.2.1）。
    end
    tenant_index :jurisdiction_capability_skips, %i[jurisdiction capability]
    tenant_index :jurisdiction_capability_skips, %i[event_kind occurred_at]

    create_tenant_table :contract_liability_entries, comment: "合約負債分錄（HK 首發即使用，56 §B.3.1 J-01）" do |t|
      ...
      t.string :basis, null: false, limit: 128
    end
    tenant_index :contract_liability_entries, %i[source_type source_id direction], unique: true
  end
