...
- 🔴 **修復研究／等待自動化／自動合併**（2026-08-18 新增鐵律 16/17/18，全文在 CLAUDE.md）：
  ...
  **機械 CI 全綠** ∧ 判詞格式機械驗證（每項存在型判定，沒跑≠零意見），
  改 `.github/workflows/` 任何檔／機械閘門判準（ci.yml・`config/ci.rb` 的 step 所引用的
  任何判準檔——scripts/ 全部腳本為主，另含 `.rubocop.yml`、`package.json` scripts、
  `spec/` 等，舉例非窮舉）／CLAUDE.md／AGENTS.md 的 PR 一律人工；人工合併類 PR
  （含 18.4 啟用前過渡期的一切 PR）合併完成前其依賴鏈不自動前進；
