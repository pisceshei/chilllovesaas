...
> 🔴 **D83（2026-09-04 使用者裁定）**：使用者授權 Claude Code 工作階段**代行合併（含 18.3 清單的 PR）**，CI 兩測綠即合、
> 不需等使用者確認（D40 模式下的合併前提＝`quality`＋`test` 綠；仍不是 workflow 自動合併）。
...
- 🔴 **渲染 1:1**（2026-09-04 新增鐵律 22，全文在 CLAUDE.md）：編輯器 live preview 與買家前台的實際渲染
  ...
  🔴 D83（2026-09-04）：判準**精確到 CSS 級別**（computed style 逐屬性相等）；已購授權主題 Ella 7.2.0 的原檔（Liquid／CSS／JS／資產／locale）
  **直接完整複製**到我方渲染，使用者裁定無版權問題。
  🔴 **「一份 worklog（不另建「第 M 輪」）」這一條對規則生效前已開的 PR 不追溯；其餘條文（分層、更正註、閘門、ledger）照舊不豁免**：判準與射程邊界見 `docs/DECISIONS.md` **D39**（2026-08-22 使用者裁定）。
