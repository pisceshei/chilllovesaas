...
🔴 **根因不是誰不夠謹慎，是結構性的**：`claude-review.yml` 的 `allowedTools` 原本零個網路
工具（2026-08-19 已補 `WebSearch`／`WebFetch`），Codex 的引證措辭也清一色是
