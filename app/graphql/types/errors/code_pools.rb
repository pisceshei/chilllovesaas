...
module Types
  ...
  module Errors
    ...
    module CodePools
      ...
      # 併發／樂觀鎖／冪等——🔴 **本輪新開的類別**。
      #
      # 為什麼不能塞進 `COMMON`：`COMMON` 的 24 個**以欄位級輸入驗證為主**
      # （「這個值太長」「這個值不在清單裡」），而併發衝突講的是
      # 「你看到的狀態已經過期」——它不是輸入錯誤，使用者重送一模一樣的輸入是合理的。
      # 前端對兩者的處置也完全不同：前者標紅欄位，後者要跳「重新載入／覆蓋儲存」。
      #
      # ⚠️ **原文寫的是「那 24 個**全是**欄位級輸入驗證」，那句話不成立**（2026-08-15 修）。
      # `COMMON` 裡至少這幾個不是欄位級輸入驗證：
      # `CREATION_FAILED`／`INTERNAL_ERROR`（執行失敗）、`MISSING_PERMISSION`（授權）、
      # `FEATURE_NOT_ENABLED`（方案分層）、`NOT_FOUND`（查無資源）、
      # `NOT_EDITABLE`／`INVALID_STATE`（狀態不允許）。
      # 🔴 **但這不動搖本段的結論**：不塞進 `COMMON` 的真正理由是**前端處置不同**
      # （標紅欄位 vs 跳重新載入），不是「COMMON 剛好同質」。
      # 把理由寫成一個**可以被一次反例推翻**的全稱句，是這句話真正的問題。
      #
