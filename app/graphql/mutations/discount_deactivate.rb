...
module Mutations
  # 折扣停用（17-F4.4 語義：有 applications 的折扣不硬刪——停用是常規出口）。
  class DiscountDeactivate < BaseMutation
    graphql_name "DiscountDeactivate"
    description "停用折扣（status → archived；設定保留）。"
    ...
    def resolve(id:)
      ...
      ActsAsTenant.with_tenant(shop) { record.update!(status: "archived") }
