...
module Mutations
  # 更新自動折扣（basic 形）（官方同名 mutation 對位；輸入形＝ours 合流 DiscountBasicInput）。
  class DiscountAutomaticBasicUpdate < BaseMutation
    graphql_name "DiscountAutomaticBasicUpdate"
    description "更新自動折扣（basic 形）"
    ...
    def resolve(id:, input:)
      ...
      return error("id", "method 不符（automatic 專用 mutation）。", "INVALID") unless record.method == "automatic"
