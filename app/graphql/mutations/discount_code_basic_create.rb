...
module Mutations
  # 建立折扣碼（basic 形）（官方同名 mutation 對位；輸入形＝ours 合流 DiscountBasicInput）。
  class DiscountCodeBasicCreate < BaseMutation
    graphql_name "DiscountCodeBasicCreate"
    description "建立折扣碼（basic 形）"
    ...
    def resolve(input:)
      ...
      record = ActsAsTenant.with_tenant(shop) do
        Discount.new(shop_id: shop.id, method: "code", status: "active",
