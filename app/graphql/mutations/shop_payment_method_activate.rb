...
module Mutations
  # 啟用 manual 付款方式（G6-3 步 2；86 §3 Deactivate 語義的逆向——設定保留，
  # 隨時可再啟用；activate/deactivate 只翻 active 旗標不動文案）。
  class ShopPaymentMethodActivate < BaseMutation
    graphql_name "ShopPaymentMethodActivate"
    description "啟用 manual 付款方式（結帳頁 Payment 段隨之顯示）。"
    ...
    def resolve(id:)
      ...
      ActsAsTenant.with_tenant(shop) { record.update!(active: true) }
      payload(record)
    end
