...
module Mutations
  # 停用 manual 付款方式（G6-3 步 2；86 §3 Deactivate 確認逐字「Your account details
  # will be saved and you can reactivate … at any time.」＝停用保留設定值、不刪列）。
  class ShopPaymentMethodDeactivate < BaseMutation
    graphql_name "ShopPaymentMethodDeactivate"
    description "停用 manual 付款方式（設定保留、可隨時再啟用）。"
    ...
    def resolve(id:)
      ...
      ActsAsTenant.with_tenant(shop) { record.update!(active: false) }
      payload(record)
    end
